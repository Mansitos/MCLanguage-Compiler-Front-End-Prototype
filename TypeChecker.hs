...
getRealTypeDef (TResult env (Pointer t depth) pos) def = case def of
                                                        Abs.ExpressionBracketD p exp -> if depth==checkDef_ exp then (TResult env t pos) else TError ["depth different at "++show pos]
