...
--genTacWhileDoStatement (Abs.WhileStateCtrlDo res ctrl state) n l k (w,j)  =
--genTacWhileDoStatement (Abs.WhileStateCtrlWDo res ctrl b) n l k (w,j)     =
...
genTacConditionalStatement (Abs.ConditionalStatementCtrlThen res ctrlState state elseState) n l k (w,j) =   let ctrlStatement = genTacControlStatement ctrlState n l k (w,j) in
                                                                                                                let statTac = genTacStatement state n l k (w,j) res in
                                                                                                                    let flagValue = checkControlDeclarationStatement ctrlState in   -- get the result of the control declaration statement! valid declaration = true; invalid declaration = false flag!
                                                                                                                        case elseState of                                           -- We already know at COMPILE-TIME which code block (then or else) will be executed; there's no need of jumps
                                                                                                                            ...
                                                                                                                            (Abs.ElseStateEmpty _)  -> let tac = case flagValue of
                                                                                                                                                                    True  -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 statTac)))) [] -- we know then_code must be executed, tac = ctrldecl tac + tac of then block
                                                                                                                                                                    False -> TAC [] [] -- we know that else_code must be executed but else is empty! tac = empty
                                                                                                                                                                        in ((Abs.ConditionalStatementCtrlThen tac (sel1 ctrlStatement) (sel1 statTac) (Abs.ElseStateEmpty (TAC [] []))),(sel2 statTac),(sel3 statTac)) -- if ... then ...
                                                                                                                            -- if CTRLDECL FLAG then CODE else CODE  
                                                                                                                            (Abs.ElseState _ elsestats) -> let elseStatesTac = genTacStatement elsestats (sel2 statTac) l (sel3 statTac) (w,j) res in
                                                                                                                                                                let nextLab = newLabel "next" (sel3 elseStatesTac) in
                                                                                                                                                                    let tac = case flagValue of
                                                                                                                                                                                True  -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 statTac)))) [] -- we know then_code  must be executed, tac = ctrldecl tac + tac of then block
                                                                                                                                                                                False -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 elseStatesTac)))) [] -- we know else_code must be executed, tac = ctrldecl tac + tac of else block
                                                                                                                                                                                            in ((Abs.ConditionalStatementCtrlThen tac (sel1 ctrlStatement) (sel1 statTac) (Abs.ElseState (TAC [] []) (sel1 elseStatesTac)),(sel2 elseStatesTac),(sel3 elseStatesTac)+1)) -- if expr then ... else ...        
genTacConditionalStatement (Abs.ConditionalStatementCtrlWThen res ctrlState b@(Abs.BlockStatement _ states) elseState) n l k (w,j)    = let ctrlStatement = genTacControlStatement ctrlState n l k (w,j) in
                                                                                                                                            let statsTac = genTacStatements states n l k (w,j) in
                                                                                                                                                let flagValue = checkControlDeclarationStatement ctrlState in   -- get the result of the control declaration statement! valid declaration = true; invalid declaration = false flag!
                                                                                                                                                    case elseState of                                           -- We already know at COMPILE-TIME which code block (then or else) will be executed; there's no need of jumps
                                                                                                                                                        -- if CTRLDECL FLAG then CODE                           -- We ignore else code if the ctrl declaration was valid, and the opposite!
                                                                                                                                                        (Abs.ElseStateEmpty _)  -> let tac = case flagValue of
                                                                                                                                                                                                True  -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statements_content (sel1 statsTac)))) [] -- we know then_code must be executed, tac = ctrldecl tac + tac of then block
                                                                                                                                                                                                False -> TAC [] [] -- we know that else_code must be executed but else is empty! tac = empty
                                                                                                                                                                                                    in ((Abs.ConditionalStatementCtrlWThen tac (sel1 ctrlStatement) (Abs.BlockStatement (TAC [] []) (sel1 statsTac)) (Abs.ElseStateEmpty (TAC [] []))),(sel2 statsTac),(sel3 statsTac)) -- if ... then ...
                                                                                                                                                        -- if CTRLDECL FLAG then CODE else CODE  
                                                                                                                                                        (Abs.ElseState _ elsestats) -> let elseStatesTac = genTacStatement elsestats (sel2 statsTac) l (sel3 statsTac) (w,j) res in
                                                                                                                                                                                            let nextLab = newLabel "next" (sel3 elseStatesTac) in
                                                                                                                                                                                                let tac = case flagValue of
                                                                                                                                                                                                            True  -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statements_content (sel1 statsTac)))) [] -- we know then_code  must be executed, tac = ctrldecl tac + tac of then block
                                                                                                                                                                                                            False -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 elseStatesTac)))) [] -- we know else_code must be executed, tac = ctrldecl tac + tac of else block
                                                                                                                                                                                                                        in ((Abs.ConditionalStatementCtrlWThen tac (sel1 ctrlStatement) (Abs.BlockStatement (TAC [] []) (sel1 statsTac)) (Abs.ElseState (TAC [] []) (sel1 elseStatesTac)),(sel2 elseStatesTac),(sel3 elseStatesTac)+1)) -- if expr then ... else ...        
...
genTacControlStatement (Abs.CtrlDecStateVar res@(TResult _ ty _) ident@(Abs.Ident id resi@(TResult _ _ pos)) typepart@(Abs.TypePart (TResult _ typePart _) _) expr) n l k (w,j) = case ty of 
                                                                                                                                            B_type Type_Void -> let expression = genTacExpression expr n l k (w,j) res in 
                                                                                                                                                                    ((Abs.CtrlDecStateVar (TAC [TacAssignNullOp (buildIDAddr pos id) (genDefaultInitAddr typePart) typePart,    -- the ctrl decl statement was invalid; so ctrl var is initialized with default value
                                                                                                                                                                                                TacComment "CtrlDeclStat was invalid; then code is ignored."] [])           
                                                                                                                                                                                                (Abs.Ident id (TAC [] [])) (Abs.TypePart (TAC [] []) (Abs.TypeExpression (TAC [] []) (Abs.PrimitiveTypeVoid (TAC [] [])))) (sel1 expression)),n,k)  -- it wasn't a correct ctrldecl assignment! so initialize at def. value!
                                                                                                                                            _                -> let expression = genTacExpression expr n l k (w,j) res in
                                                                                                                                                                    let exprAddr = sel4 expression in
                                                                                                                                                                        ((Abs.CtrlDecStateVar (TAC (code (expression_content (sel1 expression)) ++              -- tac code of rval expr evaluation
                                                                                                                                                                                                   ...
                                                                                                                                                                                                   TacComment "CtrlDeclStat was valid; else code is ignored."])
                                                                                                                                                                                                   []) (Abs.Ident id (TAC [] [])) (Abs.TypePart (TAC [] []) (Abs.TypeExpression (TAC [] []) (Abs.PrimitiveTypeVoid (TAC [] [])))) (sel1 expression)),n,k)  -- it was a correct ctrldecl assignment                                                                    
--genTacControlStatement (Abs.CtrlDecStateConst res@(TResult _ ty _) id typepart expr) =
-- ident@(Abs.Ident id resi)) n l k (w,j) tres       = (Abs.IdentifierSingle (TAC [] []) (Abs.Ident id (TAC [] [])),n,k,[buildIDAddr pos id])
