...
genTacConditionalStatement (Abs.ConditionalStatementCtrlThen res ctrlState state elseState) n l k (w,j) =   let ctrlStatement = genTacControlStatement ctrlState n l k (w,j) in
                                                                                                                let statTac = genTacStatement state n l k (w,j) res in
                                                                                                                    let flagValue = checkControlDeclarationStatement ctrlState in
                                                                                                                        case elseState of 
                                                                                                                            -- if CTRLDECL FLAG then CODE
                                                                                                                            (Abs.ElseStateEmpty _)  -> let tac = case flagValue of
                                                                                                                                                                    True  -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 statTac)))) [] -- we know then must be executed, tac = ctrldecl tac + tac of then block
                                                                                                                                                                    False -> TAC [] [] -- we know that else must be executed but else is empty! tac = empty
                                                                                                                                                                        in ((Abs.ConditionalStatementCtrlThen tac (sel1 ctrlStatement) (sel1 statTac) (Abs.ElseStateEmpty (TAC [] []))),(sel2 statTac),(sel3 statTac)) 
                                                                                                                            -- if CTRLDECL FLAG then CODE else CODE  
                                                                                                                            (Abs.ElseState _ elsestats) -> let elseStatesTac = genTacStatement elsestats (sel2 statTac) l (sel3 statTac) (w,j) res in
                                                                                                                                                                let nextLab = newLabel "next" (sel3 elseStatesTac) in
                                                                                                                                                                    let tac = case flagValue of
                                                                                                                                                                                True  -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 statTac)))) [] -- we know then must be executed, tac = ctrldecl tac + tac of then block
                                                                                                                                                                                False -> TAC ((code (ctrldecstatement_content (sel1 ctrlStatement)))++(code (statement_content (sel1 elseStatesTac)))) [] -- we know else must be executed, tac = ctrldecl tac + tac of else block
...
--genTacConditionalStatement (Abs.ConditionalStatementCtrlWThen res ctrlState b elseState) n l k (w,j)    =
...
genTacControlStatement (Abs.CtrlDecStateVar res@(TResult _ ty _) ident@(Abs.Ident id resi@(TResult _ _ pos)) typepart expr) n l k (w,j) = case ty of 
                                                                                                                                            B_type Type_Void -> let expression = genTacExpression expr n l k (w,j) res in 
                                                                                                                                                                    ((Abs.CtrlDecStateVar (TAC [TacAssignNullOp (buildIDAddr pos id) (genDefaultInitAddr (B_type Type_Integer)) (B_type Type_Integer)] []) (Abs.Ident id (TAC [] [])) (Abs.TypePart (TAC [] []) (Abs.TypeExpression (TAC [] []) (Abs.PrimitiveTypeVoid (TAC [] [])))) (sel1 expression)),n,k)  -- it wasn't a correct ctrldecl assignment! so initialize at def. value!
                                                                                                                                            _                -> let expression = genTacExpression expr n l k (w,j) res in
                                                                                                                                                                    let exprAddr = sel4 expression in
                                                                                                                                                                        ((Abs.CtrlDecStateVar (TAC (code (expression_content (sel1 expression)) ++
                                                                                                                                                                                                   [TacAssignNullOp (buildIDAddr pos id) exprAddr (B_type Type_Integer)])
