...
buildTacEntriesForVarsDecl_ :: [Address] -> [Address] -> Type -> Prelude.Integer -> Prelude.Integer -> Prelude.Integer -> ([TACEntry],Prelude.Integer)
buildTacEntriesForVarsDecl_ [x] [r] (Array t d) dim n c = buildTacEntriesForVarsDecl_ [x] [r] t dim n c
buildTacEntriesForVarsDecl_ [x] [r] t dim n c = let temp = newTemp c in
                                                    ([TacAssignNullOp False temp (buildArrayId r n dim) t]++(buildTacEntriesForVarsDecl [x] [temp] t dim n),c+1)
buildTacEntriesForVarsDecl_ [x] (r:rs) (Array t d) dim n c = buildTacEntriesForVarsDecl_ [x] (r:rs) t dim n c
buildTacEntriesForVarsDecl_ [x] (r:rs) t dim n c = let temp = newTemp c in
                                                        let next = (buildTacEntriesForVarsDecl_ [x] rs t dim (n+dim) (c+1)) in 
                                                            ([TacAssignNullOp False temp (buildArrayId r n dim) t]++(buildTacEntriesForVarsDecl [x] [temp] t dim n)++(sel1 next),sel2 next)
...
buildTacEntriesForVarsDecl [x] [r] ty dim n = case r of     -- single, not pointer
    AddrNULL -> [TacAssignNullOp False (buildArrayId x n dim) (genDefaultInitAddr ty) ty]
    AddrAddress addr -> case addr of
...
buildTacEntriesForVarsDecl [x] (r:rs) ty dim n = case r of
    AddrNULL -> [TacAssignNullOp False (buildArrayId x n dim) (genDefaultInitAddr ty) ty] ++ buildTacEntriesForVarsDecl [x] rs ty dim (n+dim)
    _ -> [TacAssignNullOp False (buildArrayId x n dim) r ty] ++ buildTacEntriesForVarsDecl [x] rs ty dim (n+dim)
------------------------------------------------------ 
...
genAbsoluteType :: Type -> Type
genAbsoluteType (Array t dim) = genAbsoluteType t
genAbsoluteType (Pointer t dim) = genAbsoluteType t
genAbsoluteType t = t
...
buildAssignTac assignOp (leftAddr:xs) (a:arr) rightAddr t = case assignOp of
                                                        (Abs.AssignOperationEq _ )         -> case t of
                                                                                                ...
                                                                                                _ ->  [TacAssignNullOp False leftAddr rightAddr t] ++ [a] ++ buildAssignTac assignOp xs arr rightAddr t
...
buildAssignTac assignOp (leftAddr:xs) [] rightAddr t = case assignOp of
                                                        (Abs.AssignOperationEq _ )         -> case t of
                                                                                                ...
                                                                                                _ -> [TacAssignNullOp False   leftAddr rightAddr t] ++ buildAssignTac assignOp xs [] rightAddr t
...
findArray ((Variable ty@(Array _ _) pos mode override s check):xs) = [Variable ty pos mode override s check]
findArray (x:xs) = case x of
                    Variable (Pointer t depth) pos mode override s check -> let finder = findArray [Variable t pos mode override s check] in
                                                                        case finder of
                                                                            [] -> findArray xs
                                                                            _ -> [x]
                    Variable t pos mode override s check -> findArray xs
findArray [] = []
...
isAndOrOp :: Abs.EXPRESSION a -> Prelude.Bool
isAndOrOp (Abs.ExpressionBinaryAnd _ _ _) = True
isAndOrOp (Abs.ExpressionBinaryOr _ _ _) = True
isAndOrOp (Abs.ExpressionBracket _ exp ) = isAndOrOp exp 
isAndOrOp (Abs.ExpressionBinaryEq _ exp1 exp2) = isAndOrOp exp1  || isAndOrOp exp2 
isAndOrOp (Abs.ExpressionBinaryNotEq _ exp1 exp2) = isAndOrOp exp1  || isAndOrOp exp2 
isAndOrOp (Abs.ExpressionBinaryGratherEq _ exp1 exp2) = isAndOrOp exp1  || isAndOrOp exp2 
isAndOrOp (Abs.ExpressionBinaryGrather _ exp1 exp2) = isAndOrOp exp1  || isAndOrOp exp2 
isAndOrOp (Abs.ExpressionBinaryLessEq _ exp1 exp2) = isAndOrOp exp1  || isAndOrOp exp2 
isAndOrOp (Abs.ExpressionBinaryLess _ exp1 exp2) = isAndOrOp exp1  || isAndOrOp exp2 
isAndOrOp (Abs.ExpressionUnary _ _ def) = isAndOrOp_ def 
isAndOrOp (Abs.ExpressionCast _ def _) = isAndOrOp_ def 
isAndOrOp _ = False

isAndOrOp_ :: Abs.DEFAULT a -> Prelude.Bool
isAndOrOp_ (Abs.ExpressionBracketD _ exp )  = isAndOrOp exp
isAndOrOp_ (Abs.ExpressionUnaryD _ _ def)  = isAndOrOp_ def
isAndOrOp_ (Abs.ExpressionCastD _ def _)  = isAndOrOp_ def
isAndOrOp_ _ = False

generateTac :: Prelude.Integer -> Address -> Prelude.Integer -> Prelude.Integer -> Type -> Prelude.Integer -> ([TACEntry],Prelude.Integer,[Address],Prelude.Integer)
generateTac n adr c dim t 1 = let temp = newTemp n in
                                ([TacAssignNullOp False temp (buildArrayId adr c dim) t],n+1,[temp],c+dim)
generateTac n adr c dim t l = let temp = newTemp n in
                                let next = generateTac (n+1) adr (c+dim) dim t (l-1) in
                                    ([TacAssignNullOp False temp (buildArrayId adr c dim) t]++(sel1 next),sel2 next,[temp]++(sel3 next),sel4 next)

getSonTypeExp :: Abs.TYPEEXPRESSION a -> Abs.TYPEEXPRESSION a
getSonTypeExp (Abs.TypeExpressionArraySimple _ _ typeexpf) = getSonTypeExp_ typeexpf
getSonTypeExp (Abs.TypeExpressionArray _ _ typeexpf) = getSonTypeExp_ typeexpf
getSonTypeExp t = t

getSonTypeExp_ :: Abs.TYPEEXPRESSIONFUNC a -> Abs.TYPEEXPRESSION a
...
getDimFromExpFTAC :: Abs.TYPEEXPRESSIONFUNC a -> Abs.EXPRESSION a -> Env -> Prelude.Integer
getDimFromExpFTAC (Abs.TypeExpressionArrayOfPointer _ _) exp env = 0
getDimFromExpFTAC (Abs.TypeExpressionFunction _ typeexp) exp env = getDimFromTypeExpTAC typeexp exp env

getDimFromTypeExpTAC :: Abs.TYPEEXPRESSION a -> Abs.EXPRESSION a -> Env -> Prelude.Integer
getDimFromTypeExpTAC (Abs.TypeExpressionArraySimple _ range expf) exp env = let next = getDimFromExpFTAC expf exp env in
                                                                    if next==0 
                                                                        then getDimFromRange range
                                                                        else getDimFromRange range * getDimFromExpFTAC expf exp env
getDimFromTypeExpTAC (Abs.TypeExpressionArray _ range expf) exp env = let next = getDimFromExpFTAC expf exp env in
                                                                    if next==0 
                                                                        then getDimFromRange range
                                                                        else getDimFromRange range * getDimFromExpFTAC expf exp env
...
getDimFromPrimTAC p@(Abs.TypeArray _ prim) (Abs.ExpressionBracket _ exp) env = getDimFromPrimTAC p exp env
getDimFromPrimTAC (Abs.TypeArray _ prim) (Abs.ExpressionIdent _ (Abs.Ident id _) index) env = case Data.Map.lookup id env of
                                                                                                Just (e:es) -> case findEntryOfType (e:es) "var" of
                                                                                                                [] -> 0
                                                                                                                [Variable (Array t dim) _ _ _ s check] -> case index of 
                                                                                                                                                        Abs.ArrayIndexElementEmpty _ -> getNElems s (toInteger(length s))
                                                                                                                                                        Abs.ArrayIndexElement _ exp -> head s
                                                                                                                                                        Abs.ArrayIndexElements _ exps exp  -> getNElems s (1+(countSquares exps))
                                                                                                                _ -> 0

getDimFromPrimTAC _ _ _ = 0
                                                                                                                ...
                                                                                                                let vardecAddrs = sel5 vardecTac in -- variable addresses # >1
                                                                                                                    let initAddr = sel6 vardecTac in
                                                                                                                        let dim = sel4 vardecTac in
                                                                                                                            if head initAddr == AddrAddress "array"
                                                                                                                                then
                                                                                                                                    let idBuilder = buildTacEntriesForVarsDecl_ vardecAddrs (tail initAddr) ty dim 0 (sel2 vardecTac) in
                                                                                                                                        (Abs.VariableDeclarationStatement (mergeTacs [vardecContent,
                                                                                                                                                                              (TAC (sel1 idBuilder) []),
                                                                                                                                                                              tipoContent])
                                                                                                                                                                              (sel1 tipoTac) (sel1 vardecTac) ,(sel2 idBuilder),(sel3 vardecTac),AddrNULL)
                                                                                                                                else
                                                                                                                                    let idBuilder = buildTacEntriesForVarsDecl vardecAddrs initAddr ty dim 0 in
                                                                                                                                        (Abs.VariableDeclarationStatement (mergeTacs [vardecContent,
                                                                                                                                                                                      (TAC idBuilder []),
                                                                                                                                                                                      tipoContent])
...
genTacStatement (Abs.AssignmentStatement resres@(TResult _ t _) lval assignOp exp) n l k (w,j) = if isAndOrOp exp
                                                                                                    then
                                                                                                        let label = newLabel "endSet" k in
                                                                                                            let temp = case t of
                                                                                                                        B_type Type_Real -> newTemp (n+2)
                                                                                                                        B_type Type_Integer -> newTemp (n+1)
                                                                                                                        B_type Type_Boolean -> newTemp n in
                                                                                                                            ...
                                                                                                                            let exprAddr = sel4 rightVal in
                                                                                                                                let leftValAddrs = sel4 leftVal in
                                                                                                                                    let lTac = (code (lvalueexpression_content (sel1 leftVal))) in
                                                                                                                                        ...
                                                                                                                                        case t of
                                                                                                                                                B_type Type_Real -> let tacCode = code (expression_content (sel1 rightVal)) in
                                                                                                                                                                        let lastEl1 = last tacCode in
                                                                                                                                                                            let remainEl1 = init tacCode in
                                                                                                                                                                                let lastEl2 = last remainEl1 in
                                                                                                                                                                                    let remainEl2 = init remainEl1 in
                                                                                                                                                                                        ((Abs.AssignmentStatement (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                            TAC remainEl2 [],
                                                                                                                                                                                            (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                            (TAC [TacLabel label] []),
                                                                                                                                                                                            TAC [lastEl2] [],
                                                                                                                                                                                            TAC [lastEl1] [],
                                                                                                                                                                                            (TAC assignTac [])
                                                                                                                                                                                            ]) (sel1 leftVal) (genTacAssignOp assignOp) (sel1 rightVal)),newC,(sel3 rightVal)+1,AddrNULL)
                                                                                                                                                B_type Type_Integer -> let tacCode = code (expression_content (sel1 rightVal)) in
                                                                                                                                                                        let lastEl = last tacCode in
                                                                                                                                                                            let remainEl = init tacCode in
                                                                                                                                                                                ((Abs.AssignmentStatement (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                    TAC remainEl [],
                                                                                                                                                                                    (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                    (TAC [TacLabel label] []),
                                                                                                                                                ...
                                                                                                                                                B_type Type_Boolean -> ((Abs.AssignmentStatement (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                    ...
                                                                                                                                    let statements = (genTacStatements states n newL k (w,j)) in
                                                                                                                                        let newC = sel2 statements in
                                                                                                                                            let newK = sel3 statements in
                                                                                                                                                let entries = Data.Map.lookup id (getEnvFromStates states) in
                                                                                                                                                       case entries of
                                                                                                                                                           Just (x:xs) -> let entry = findEntryOfType (x:xs) "func" in
                                                                                                                                                                           let params = (case entry of
                                                                                                                                                                                                ((Function typ posv pa override):ys) -> pa) in
                                                                                                                                                                                                        ((Abs.ProcedureStatement (mergeTacs [(TAC [] [TacLabel newL]),  -- start func code label
                                                                                                                                                                                                                                             (buildTacForValResParamsInit params),-- for each valres param initialize it
...
genTacStatement (Abs.FunctionStatement res@(TResult env _ _) ident@(Abs.Ident id resI@(TResult _ _ (Pn a r c))) param tipo states) n l k (w,j) = let newL = Label (id ++ "@" ++show r++","++show c) in 
                                                                                                                                                                           ...
                                                                                                                                                                           let params = (case entry of
                                                                                                                                                                                                ((Function typ posv pa override):ys) -> pa) in
                                                                                                                                                                                                        ((Abs.FunctionStatement (mergeTacs [(TAC [] [TacLabel newL]),  -- start func code label
                                                                                                                                                                                                                                            (buildTacForValResParamsInit params),-- for each valres param initialize it
                                                                                                                                                                                                                                            (attachValResReturnCopies (TAC [] (code (statements_content (sel1 statements)))) (buildTacForValResParamsResultReturn params)), -- tac of statements inside the body (no funcs. decl. tacs.)
                                                                                                                                                                                                                                            (TAC [] [TacLabel endL]),  -- end func code label   
                                                                                                                                                                                                                                            (TAC [] (funcs (statements_content (sel1 statements)))) -- functions declared inside
...
attachValResReturnCopies :: TAC -> TAC -> TAC
attachValResReturnCopies (TAC y (x:xs)) (TAC k copiesTac) = case x of
                                                                (TacReturn _ _) -> merge2Tacs (TAC y (copiesTac++[x])) (attachValResReturnCopies (TAC [] xs) (TAC k copiesTac ))
                                                                _ -> merge2Tacs (TAC y [x]) (attachValResReturnCopies (TAC [] xs) (TAC k copiesTac))
attachValResReturnCopies _ _ = (TAC [] [])                                                             


buildTacForValResParamsInit :: [TypeChecker.Parameter] -> TAC
buildTacForValResParamsInit (p:ps) = case p of
                                      (TypeChecker.Parameter _ pos "val" id) -> merge2Tacs (TAC [] []) (buildTacForValResParamsInit ps)                                   
                                      (TypeChecker.Parameter ty pos "valres" id) -> let localAddr = buildIDAddr pos (id++"_local_copy") (fromList []) in 
                                                                                     let paramAddr = buildIDAddr pos id (fromList []) in
                                                                                        (merge2Tacs (TAC [] [TacAssignUnaryOp localAddr Point paramAddr ty,TacComment "valres-mode"]) (buildTacForValResParamsInit ps))
buildTacForValResParamsInit [] = (TAC [] [])  

buildTacForValResParamsResultReturn :: [TypeChecker.Parameter] -> TAC
buildTacForValResParamsResultReturn (p:ps) = case p of
                                                      (TypeChecker.Parameter _ pos "val" id) -> merge2Tacs (TAC [] []) (buildTacForValResParamsResultReturn ps)                   
                                                      (TypeChecker.Parameter ty pos "valres" id) -> let localAddr = buildIDAddr pos (id++"_local_copy") (fromList []) in 
                                                                                                     let paramAddr = buildIDAddr pos id (fromList []) in
                                                                                                        (merge2Tacs (TAC [] [TacAssignNullOp False paramAddr localAddr ty,TacComment "valres-mode-copy"]) (buildTacForValResParamsResultReturn ps))
buildTacForValResParamsResultReturn [] = (TAC [] [])       


getEnvFromStates :: Abs.STATEMENTS TCheckResult -> Env
getEnvFromStates (Abs.ListStatements (TResult env _ _) _ _) = env
getEnvFromStates (Abs.EmptyStatement (TResult env _ _)) = env
...
genTacLeftVal (Abs.LvalueExpressions res@(TResult env _ _) ident@(Abs.Ident id resi) arrayindex lval) n l k (w,j) = case arrayindex of  
                                                                                                    (Abs.ArrayIndexElementEmpty _) -> case Data.Map.lookup id env of
                                                                                                        Just (entry:entries) ->  case findEntryOfType (entry:entries) "var" of
                                                                                                            ((Variable _ posv _ _ _ _):xs) -> let leftAddr = buildIDAddr posv id env in
                                                                                                    ...
                                                                                                    (Abs.ArrayIndexElement _ typeindex) -> case Data.Map.lookup id env of
                                                                                                                                                Just (entry:entries) ->  case findEntryOfType (entry:entries) "var" of
                                                                                                                                                    (x:xs) -> case findArray (x:xs) of
                                                                                                                                                        [Variable ty@(Array t _) posv _ _ s check] -> let typeIndexTac = genTacTypeIndex typeindex n l k (w,j) 0 [] (-1) in
                                                                                                                                                                                                    let lvalAddr = (genTacLeftVal lval (sel2 typeIndexTac) l (sel3 typeIndexTac) (w,j)) in
                                                                                                                                                                                                        let dim = generateDimForArrayFromType ty in
                                                                                                                                                                                                            let leftAddr = buildIDAddr posv id env in
                                                                                                    ...
                                                                                                    (Abs.ArrayIndexElements _ arrayIndex typeindex ) -> case Data.Map.lookup id env of
                                                                                                                                                            Just (entry:entries) ->  case findEntryOfType (entry:entries) "var" of
                                                                                                                                                                (x:xs) ->  case findArray (x:xs) of
                                                                                                                                                                    [Variable ty@(Array t _) posv _ _ s check] -> let realT = getRealTypeFromArray t in
...
genTacTypeIndex (Abs.TypeOfIndexExpressionCallSingle res@(TResult env t pos) ident@(Abs.Ident id _) exprs) n l k (w,j) d s p = let entries = Data.Map.lookup id env in
                                                                                                                                case entries of
                                                                                                                                    Just (x:xs) -> let entry = findEntryOfType (x:xs) "func" in
                                                                                                                                                    let params = (case entry of
                                                                                                                                                                        ((Function typ posv pa override):ys) -> pa) in
                                                                                                                                                                            let exprsTac = genTacExpressions exprs params n l k (w,j) in
...
genTacTypeIndex (Abs.TypeOfIndexExpressionCall res@(TResult env t pos) typeindex ident@(Abs.Ident id _) exprs) n l k (w,j) d s p = let entries = Data.Map.lookup id env in
                                                                                                                                    case entries of
                                                                                                                                        Just (x:xs) -> let entry = findEntryOfType (x:xs) "func" in
                                                                                                                                                        let params = (case entry of
                                                                                                                                                                            ((Function typ posv pa override):ys) -> pa) in
                                                                                                                                                                                let typeindexTac = genTacTypeIndex typeindex n l k (w,j) d s (p+1) in
                                                                                                                                                                                    let exprsTac = genTacExpressions exprs params (sel2 typeindexTac) l (sel3 typeindexTac) (w,j) in
...
genTacExpressionStatement (Abs.VariableExpression res@(TResult env _ pos) ident@(Abs.Ident id resi)) n l k (w,j) = ((Abs.VariableExpression (TAC [] []) (Abs.Ident id (TAC [] []))),n,k,buildIDAddr pos id env)
genTacExpressionStatement (Abs.CallExpression res@(TResult env ty pos) call) n l k (w,j)   = let callexpression = (genTacCallExpression call n l k (w,j)) in
...
genTacCallExpression (Abs.CallExpressionParentheses res@(TResult env _ pos) ident@(Abs.Ident id resi) namedExpr) n l k (w,j) = let entries = Data.Map.lookup id env in
                                                                                                                                case entries of
                                                                                                                                    Just (x:xs) -> let entry = findEntryOfType (x:xs) "func" in
                                                                                                                                                    let params = (case entry of
                                                                                                                                                                        ((Function typ posv pa override):ys) -> pa) in
                                                                                                                                                                            let namedExpression = genTacNamedExpression namedExpr params n l k (w,j) in
                                                                                                                                                                                let namedExprTac = sel1 namedExpression in
                                                                                                                                                                                    let funcAddr = buildIDAddr pos id env in
...
genTacNamedExpression :: Abs.NAMEDEXPRESSIONLIST TCheckResult -> [Parameter] -> Prelude.Integer -> Label -> Prelude.Integer -> (Label,Label) -> (Abs.NAMEDEXPRESSIONLIST TAC, Prelude.Integer, Prelude.Integer)
genTacNamedExpression (Abs.NamedExpressionList res namedexpr@(Abs.NamedExpression _ expr)) (p:ps) n l k (w,j) = let expression = genTacExpression expr n l k (w,j) "" in
                                                                                                                    let exprTac = (sel1 expression) in
                                                                                                                        let exprAddr = (sel4 expression) in
                                                                                                                            let p_type = getTypeFromExpr expr in
                                                                                                                                case p of
                                                                                                                                    (TypeChecker.Parameter _ _ "val" _) -> ((Abs.NamedExpressionList (mergeTacs [(expression_content exprTac),         -- evaluation of the param
                                                                                                                                                                                                                 (TAC [TacParam exprAddr p_type False] [])])    -- param tac
                                                                                                                                                                                                                 (Abs.NamedExpression (TAC [] []) exprTac)),(sel2 expression),(sel3 expression))
                                                                                                                                    (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp (sel2 expression) in ((Abs.NamedExpressionList (mergeTacs [(expression_content exprTac),         -- evaluation of the param
                                                                                                                                                                                                                                                            (TAC [TacPointRef temp exprAddr,TacComment "valres-mode"] []),    -- param tac
                                                                                                                                                                                                                                                            (TAC [TacParam temp p_type True] [])])    -- param tac
                                                                                                                                                                                                                                                            (Abs.NamedExpression (TAC [] []) exprTac)),(sel2 expression)+1,(sel3 expression))
                                                                                                                                    _ -> ((Abs.NamedExpressionList (mergeTacs [(expression_content exprTac),         -- evaluation of the param
                                                                                                                                                                                (TAC [TacParam exprAddr p_type False] [])])    -- param tac
                                                                                                                                                                                (Abs.NamedExpression (TAC [] []) exprTac)),(sel2 expression),(sel3 expression))
genTacNamedExpression (Abs.NamedExpressionLists res namedexpr@(Abs.NamedExpression _ expr) namedexprs) (p:ps) n l k (w,j) = let expression = genTacExpression expr n l k (w,j) "" in
                                                                                                                                let namedExpressions = genTacNamedExpression namedexprs ps (sel2 expression) l (sel3 expression) (w,j) in
                                                                                                                                            ...
                                                                                                                                            let exprAddr = (sel4 expression) in
                                                                                                                                                let p_type = getTypeFromExpr expr in
                                                                                                                                                    case p of
                                                                                                                                                        ...
                                                                                                                                                        (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp (sel2 namedExpressions) in ((Abs.NamedExpressionList (mergeTacs [(expression_content exprTac),         -- evaluation of the param
                                                                                                                                                                                                                                                                                      (TAC [TacPointRef temp exprAddr,TacComment "valres-mode"] []),    -- param tac
                                                                                                                                                                                                                                                                                      (TAC [TacParam temp p_type True] []),    -- param tac     -- param tac
                                                                                                                                                                                                                                                                                      (namedexpressionlist_content namedExprsTac) -- tac of other params (recursive call)
                                                                                                                                                                                                                                                                                      ])
                                                                                                                                                                                                                                                                                      (Abs.NamedExpression (TAC [] []) exprTac)),(sel2 namedExpressions)+1,(sel3 namedExpressions))
                                                                                                                                                        _ ->  ((Abs.NamedExpressionList (mergeTacs [(expression_content exprTac),         -- evaluation of the param
                                                                                                                                                                                                    (TAC [TacParam exprAddr p_type False] []),     -- param tac
                                                                                                                                                                                                    (namedexpressionlist_content namedExprsTac) -- tac of other params (recursive call)
                                                                                                                                                                                                    ])
...
genTacForStatement (Abs.ForStateIndexDo res@(TResult env _ _) index@(Abs.IndexVarDeclaration _ ident@(Abs.Ident id resi@(TResult _ _ pos))) rangexp stat) n l k (w,j) =
                                                                        let rangeExp = (genTacRangeExpr rangexp n l k (w,j)) in -- for range do stats
                                                                            let guardLabel = newLabel "guard" (sel3 rangeExp) in
                                                                                let nextLabel = newLabel "next" ((sel3 rangeExp)+1) in
                                                                                    let statement = (genTacStatement stat (sel2 rangeExp) l ((sel3 rangeExp )+2) (nextLabel,guardLabel)) in
                                                                                                ...
                                                                                                case rangeExpTac of
                                                                                                    (Abs.RangeExpressionSingle tac expr1 expr2) -> let expr1Addr = sel4 rangeExp in
                                                                                                                                                    let expr2Addr = sel5 rangeExp in
                                                                                                                                                     let guardAddr = buildIDAddr pos id env in  -- getting guard variable address
                                                                                                                                                        ((Abs.ForStateIndexDo (mergeTacs [(rangeexp_content rangeExpTac),                   -- rangeExpr TACS (evaluation of expr1 and expr2 code)
                                                                                                                                                                                        (TAC [TacAssignNullOp False guardAddr expr1Addr (B_type Type_Integer)] []),  -- guard initialization to expr1 val
                                                                                                                                                                                        ...
                                                                                                                                                                                        (TAC [TacRelConditionalJump l True LeqInt guardAddr expr2Addr] []),  -- check of guard <= end (relation jump)
...
genTacForStatement (Abs.ForStateIndexWDo res@(TResult env _ _) index@(Abs.IndexVarDeclaration _ ident@(Abs.Ident id resi@(TResult _ _ pos))) rangexp b@(Abs.BlockStatement _ stats)) n l k (w,j) =
                                                                            let rangeExp = (genTacRangeExpr rangexp n l k (w,j)) in -- for range do stats
                                                                                let guardLabel = newLabel "guard" (sel3 rangeExp) in
                                                                                    let nextLabel = newLabel "next" ((sel3 rangeExp)+1) in
                                                                                        let statements = (genTacStatements stats (sel2 rangeExp) l ((sel3 rangeExp )+2) (nextLabel,guardLabel)) in
                                                                                                                                                    ...
                                                                                                                                                    let expr2Addr = sel5 rangeExp in
                                                                                                                                                     let guardTempAddr = (newTemp (sel2 rangeExp)) in -- generating temp variable for guard address
                                                                                                                                                        ((Abs.ForStateExprDo (mergeTacs [(rangeexp_content rangeExpTac),                       -- rangeExpr TACS (evaluation of expr1 and expr2 code)
                                                                                                                                                                                        (TAC [TacAssignNullOp False guardTempAddr expr1Addr (B_type Type_Integer)] []),  -- guard temp initialization to expr1 val
                                                                                                                                                                                        (TAC [TacJump guardLabel] []),                         -- jump to guard label
                                                                                                                                                                                        (TAC [TacLabel l] []),                                 -- body label              
                                                                                                                                                                                        (statement_content statTac),                           -- body TAC code
                                                                                                                                                                                        (TAC [TacAssignBinaryOp guardTempAddr IntAdd guardTempAddr (AddrInt 1) (B_type Type_Integer)] []), -- guard++
                                                                                                                                                                                        (TAC [TacLabel guardLabel] []),                        -- guard label
                                                                                                                                                                                        (TAC [TacRelConditionalJump l True LeqInt guardTempAddr expr2Addr] []),  -- check of guard <= end (relation jump)
                                                                                                                                                                                        (TAC [TacLabel nextLabel] [])                          -- end of for
...
genTacWhileDoStatement (Abs.WhileStateSimpleDo res expr stat) n l k (w,j) = let guardExpr = (genTacExpression expr n l k (w,j) "wor") in 
                                                                                let exprTac = sel1 guardExpr in
                                                                                    let guardLab = newLabel "guard" ((sel3 guardExpr)+(if isAndOrOp expr then 1 else 0)) in
                                                                                        let nextLab = newLabel "next" ((sel3 guardExpr)+(if isAndOrOp expr then 2 else 1)) in
                                                                                            let statement = (genTacStatement stat (sel2 guardExpr) l ((sel3 guardExpr)+(if isAndOrOp expr then 3 else 2)) (nextLab,guardLab)) in 
                                                                                                let statTac = sel1 statement in 
                                                                                                    let exprAddr = sel4 guardExpr in
                                                                                                        if isAndOrOp expr
                                                                                                            then ((Abs.WhileStateSimpleDo (mergeTacs [(TAC [TacJump guardLab,TacLabel l] []),        -- jump to guard label + body label
                                                                                                            ...
                                                                                                            else ((Abs.WhileStateSimpleDo (mergeTacs [(TAC [TacJump guardLab,TacLabel l] []),        -- jump to guard label + body label
                                                                                                                                             (statement_content statTac),                   -- body code tac
                                                                                                                                             (TAC [TacLabel guardLab] []),                  -- guard label
                                                                                                                                             (expression_content exprTac),                  -- code for guard expression eval.
...
genTacWhileDoStatement (Abs.WhileStateSimpleWDo res expr b@(Abs.BlockStatement _ statements)) n l k (w,j) = let guardExpr = (genTacExpression expr n l k (w,j) "wor") in 
                                                                                                                let exprTac = sel1 guardExpr in
                                                                                                                    let guardLab = newLabel "guard" ((sel3 guardExpr)+(if isAndOrOp expr then 1 else 0)) in
                                                                                                                        let nextLab = newLabel "next" ((sel3 guardExpr)+(if isAndOrOp expr then 2 else 1)) in
                                                                                                                            let stats = (genTacStatements statements (sel2 guardExpr) l ((sel3 guardExpr)+(if isAndOrOp expr then 3 else 2)) (nextLab,guardLab)) in 
                                                                                                                                let statsTac = sel1 stats in 
                                                                                                                                    let exprAddr = sel4 guardExpr in
                                                                                                                                        if isAndOrOp expr
                                                                                                                                            then ((Abs.WhileStateSimpleWDo (mergeTacs [(TAC [TacJump guardLab,TacLabel l] []),     -- jump to guard label + body label   
                                                                                                                                                                              (statements_content statsTac),              -- body code tac
                                                                                                                                                                              (TAC [TacLabel guardLab] []),               -- guard label
                                                                                                                                                                              (expression_content exprTac),               -- code for guard expression eval.
                                                                                                                                                                              (TAC [TacLabel nextLab] [])])                  -- next (end of while block) label
                                                                                                                                                                              exprTac (Abs.BlockStatement (TAC [] []) statsTac)),sel2 stats, sel3 stats)
                                                                                                                                            else ((Abs.WhileStateSimpleWDo (mergeTacs [(TAC [TacJump guardLab,TacLabel l] []),     -- jump to guard label + body label   
                                                                                                                                                                              (statements_content statsTac),              -- body code tac
                                                                                                                                                                              (TAC [TacLabel guardLab] []),               -- guard label
                                                                                                                                                                              (expression_content exprTac),               -- code for guard expression eval.
                                                                                                                                                                              (TAC [TacConditionalJump l True exprAddr] []), -- conditional jump on guard tac       
...
genTacConditionalStatement (Abs.ConditionalStatementSimpleThen res exp state elseState) n l k (w,j) = let expTac = genTacExpression exp n l k (w,j) "" in 
                                                                                                        let statTac = genTacStatement state (sel2 expTac) l ((sel3 expTac)+(if isAndOrOp exp  then 1 else 0)) (w,j) in        
                                                                                                            let expTacMod = TAC (if isAndOrOp exp  then (updateTac (code (expression_content (sel1 expTac))) l )
                                                                                                                ...
                                                                                                                let expAddr = sel4 expTac in
                                                                                                                    case elseState of 
                                                                                                                        -- if EXPR then CODE
                                                                                                                        (Abs.ElseStateEmpty _)  -> if isAndOrOp exp 
                                                                                                                                                    then ((Abs.ConditionalStatementSimpleThen (TAC (removeDuplicates (code (mergeTacs [expTacMod,             -- guard code eval. tac
                                                                                                                                                                                                    (case exp of
                                                                                                                                                    ...
                                                                                                                                                    else ((Abs.ConditionalStatementSimpleThen (mergeTacs [(expression_content (sel1 expTac)),             -- guard code eval. tac
                                                                                                                                                                                                          (TAC [TacConditionalJump l False expAddr] []),     -- conditional jump if-false on guard
                                                                                                                                                                                                          (statement_content (sel1 statTac)),             -- if_true code
                                                                                                                                                                                                          (TAC [TacLabel l] [])])                            -- if_false label jump (guard jump destination)
                                                                                                                                                                                                          (sel1 expTac) (sel1 statTac) (Abs.ElseStateEmpty (TAC [] []))),(sel2 statTac),(sel3 statTac)) 
                                                                                                                        -- if EXPR then CODE else CODE  
                                                                                                                        (Abs.ElseState _ elsestats) -> if isAndOrOp exp 
                                                                                                                                                        then let elseStatesTac = genTacStatement elsestats (sel2 statTac) l (sel3 statTac) (w,j) in
                                                                                                                                                        ...
                                                                                                                                                        else let elseStatesTac = genTacStatement elsestats (sel2 statTac) l (sel3 statTac) (w,j) in
                                                                                                                                                                let nextLab = newLabel "next" (sel3 elseStatesTac) in
                                                                                                                                       ...
                                                                                                                                       let statTacs = genTacStatements statements (sel2 expTac) l ((sel3 expTac)+(if isAndOrOp exp  then 1 else 0)) (w,j) in
                                                                                                                                           let expTacMod = TAC (if isAndOrOp exp  then (updateTac (code (expression_content (sel1 expTac))) l )
                                                                                                                                                ...
                                                                                                                                                let expAddr = sel4 expTac in 
                                                                                                                                                    case elseState of
                                                                                                                                                        (Abs.ElseStateEmpty _)      -> if isAndOrOp exp 
                                                                                                                                                                                            then ((Abs.ConditionalStatementSimpleWThen (TAC (removeDuplicates (code (mergeTacs [expTacMod, -- same as previous cases, but syntax has blocks!
                                                                                                                                                                                                                                         (case exp of
                                                                                                                                                                                                                                          (Abs.ExpressionBinaryOr _ _ _) -> (TAC [TacLabel (newLabel "true_or" (sel3 expTac))] [])
                                                                                                                                                                                                                                          _ -> TAC [] []
                                                                                                                                                                                                                                         ),
                                                                                                                                                                                            ...
                                                                                                                                                                                            else ((Abs.ConditionalStatementSimpleWThen (mergeTacs [(expression_content (sel1 expTac)),             -- guard code eval. tac
                                                                                                                                                                                                                                        (TAC [TacConditionalJump l False expAddr] []),     -- conditional jump if-false on guard
                                                                                                                                                                                                                                        (statements_content (sel1 statTacs)),             -- if_true code
                                                                                                                                                                                                                                        (TAC [TacLabel l] [])])                            -- if_false label jump (guard jump destination)
                                                                                                                                                                                                                                        (sel1 expTac) (Abs.BlockStatement (TAC [][]) (sel1 statTacs)) (Abs.ElseStateEmpty (TAC [] []))),(sel2 statTacs),(sel3 statTacs))
                                                                                                                                                        (Abs.ElseState _ elsestats) -> if isAndOrOp exp 
                                                                                                                                                                                            then let elseStatesTac = genTacStatement elsestats (sel2 statTacs) l (sel3 statTacs) (w,j) in
                                                                                                                                                                                                    let nextLab = newLabel "next" (sel3 elseStatesTac) in
                                                                                                                                                                                                        ((Abs.ConditionalStatementSimpleWThen (TAC (removeDuplicates (code (mergeTacs [expTacMod, -- same as previous cases, but syntax has blocks!
                                                                                                                                                                                                                                                          (case exp of
                                                                                                                                                                                                                                                          (Abs.ExpressionBinaryOr _ _ _) -> (TAC [TacLabel (newLabel "true_or" (sel3 expTac))] [])
                                                                                                                                                                                                                                                          _ -> TAC [] []
                                                                                                                                                                                                                                                         ),
                                                                                                                                                                                            ...
                                                                                                                                                                                            else let elseStatesTac = genTacStatement elsestats (sel2 statTacs) l (sel3 statTacs) (w,j) in
                                                                                                                                                                                                    let nextLab = newLabel "next" (sel3 elseStatesTac) in
                                                                                                                                                                                                        ((Abs.ConditionalStatementSimpleWThen (mergeTacs [(expression_content (sel1 expTac)),            -- guard code eval. tac
                                                                                                                                                                                                                                                        (TAC [TacConditionalJump l False expAddr] []),     -- conditional jump if-false on guard
                                                                                                                                                                                                                                                        (statements_content (sel1 statTacs)),             -- if_true code
                                                                                                                                                                                                                                                        (TAC [TacJump nextLab] []),                        -- jump to end of if label (next label)
                                                                                                                                                                                                                                                        (TAC [TacLabel l] []),                             -- if_false label jump (guard jump destination)
...
genTacControlStatement (Abs.CtrlDecStateVar res@(TResult env ty _) ident@(Abs.Ident id resi@(TResult _ _ pos)) typepart@(Abs.TypePart (TResult _ typePart _) _) expr) n l k (w,j) = case ty of 
                                                                                                                                            B_type Type_Void -> let expression = genTacExpression expr n l k (w,j) "" in 
                                                                                                                                                                    ((Abs.CtrlDecStateVar (TAC [TacAssignNullOp False (buildIDAddr pos id env) (genDefaultInitAddr typePart) typePart]    -- the ctrl decl statement was invalid; so ctrl var is initialized with default value
                                                                                                                                            ...
                                                                                                                                            _                -> let expression = genTacExpression expr n l k (w,j) "" in
                                                                                                                                                                    let exprAddr = sel4 expression in
                                                                                                                                                                        ((Abs.CtrlDecStateVar (TAC (code (expression_content (sel1 expression)) ++              -- tac code of rval expr evaluation
                                                                                                                                                                                                   [TacAssignNullOp False (buildIDAddr pos id env) exprAddr ty])         -- the ctrl decl statement was valid; so ctrl var is initialized with the initialization expr. value
...
genTacControlStatement (Abs.CtrlDecStateConst res@(TResult env ty _) ident@(Abs.Ident id resi@(TResult _ _ pos)) typepart@(Abs.TypePart (TResult _ typePart _) _) expr) n l k (w,j) = case ty of
                                                                                                                                            B_type Type_Void -> let expression = genTacExpression expr n l k (w,j) "" in 
                                                                                                                                                                ((Abs.CtrlDecStateConst (TAC [TacAssignNullOp False (buildIDAddr pos id env) (genDefaultInitAddr typePart) typePart]    -- the ctrl decl statement was invalid; so ctrl var is initialized with default value
                                                                                                                                                                                            []) (Abs.Ident id (TAC [] [])) (Abs.TypePart (TAC [] []) (Abs.TypeExpression (TAC [] []) (Abs.PrimitiveTypeVoid (TAC [] [])))) (sel1 expression)),(sel2 expression),(sel3 expression))  -- it wasn't a correct ctrldecl assignment! so initialize at def. value!
                                                                                                                                            _                -> let expression = genTacExpression expr n l k (w,j) "" in
                                                                                                                                                                    let exprAddr = sel4 expression in
...
genTacVarDecId (Abs.VariableDeclaration res@(TResult env ty _) idlist typepart@(Abs.TypePart _ typeExp) initpart) n l k (w,j) = case initpart of
                                                                                                                         ...
                                                                                                                         let temp = newTemp n in
                                                                                                                            let label = newLabel "endSet" k in
                                                                                                                                let idlistTac = genTacIdentifierList idlist (n+1) l (k+1) (w,j) in
                                                                                                                                    let tacId = identlist_content (sel1 idlistTac) in
                                                                                                                                        let addrIdList = sel4 idlistTac in
                                                                                                                                            let exprTac = (genTacExpression expr (sel2 idlistTac) label (sel3 idlistTac) (w,j) "wor") in
                                                                                                                                                let initTac = (Abs.InitializzationPart (expression_content (sel1 exprTac)) (sel1 exprTac)) in
                                                                                                                                                    let initAddr = sel4 exprTac in
                                                                                                                                                        case ty of
                                                                                                                                                            B_type Type_Real -> let tacCode = code (expression_content (sel1 exprTac)) in
                                                                                                                                                                                    let lastEl1 = last tacCode in
                                                                                                                                                                                        let remainEl1 = init tacCode in
                                                                                                                                                                                            let lastEl2 = last remainEl1 in
                                                                                                                                                                                                let remainEl2 = init remainEl1 in
                                                                                                                                                                                                    (Abs.VariableDeclaration (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                        TAC remainEl2 [],
                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                        TAC [lastEl2] [],
                                                                                                                                                                                                        TAC [lastEl1] []
                                                                                                                                                                                                        ]) (sel1 idlistTac) (Abs.TypePart (TAC [] []) (TypeExpression (TAC [] []) (Abs.PrimitiveTypeInt (TAC [] [])))) initTac ,(sel2 exprTac),(sel3 exprTac)+1,-1,addrIdList,[sel4 exprTac])
                                                                                                                                                            B_type Type_Integer -> let tacCode = code (expression_content (sel1 exprTac)) in
                                                                                                                                                                                    let lastEl = last tacCode in
                                                                                                                                                                                        let remainEl = init tacCode in
                                                                                                                                                                                            (Abs.VariableDeclaration (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                TAC remainEl [],
                                                                                                                                                            ...
                                                                                                                                                            B_type Type_Boolean -> (Abs.VariableDeclaration (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                            (expression_content (sel1 exprTac)),
                                                                                                                                                                                            (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                            (TAC [TacLabel label] [])
                                                                                                                                                            ...
                                                                                                                                                            _ -> (Abs.VariableDeclaration (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                            (expression_content (sel1 exprTac)),
                                                                                                                                                                                                            (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                            (TAC [TacLabel label] [])
                                                                                                                                                                                                            ]) (sel1 idlistTac) (Abs.TypePart (TAC [] []) (TypeExpression (TAC [] []) (Abs.PrimitiveTypeInt (TAC [] [])))) initTac ,(sel2 exprTac),(sel3 exprTac)+1,-1,addrIdList,[temp])
                                                                                                                                ...
                                                                                                                                let addrIdList = sel4 idlistTac in
                                                                                                                                    let exprTac = (genTacExpression expr (sel2 idlistTac) l (sel3 idlistTac) (w,j) "or") in
                                                                                                                                        let initTac = (Abs.InitializzationPart (expression_content (sel1 exprTac)) (sel1 exprTac)) in
                                                                                                                                            let initAddr = sel4 exprTac in
                                                                                                                                                let dim = generateDimForArray typeExp in
                                                                                                                                                    let length = getDimFromTypeTAC typepart expr env in
                                                                                                                                                        case typeExp of 
                                                                                                                                                            Abs.TypeExpressionArraySimple _ _ _ -> (Abs.VariableDeclaration (expression_content (sel1 exprTac)) (sel1 idlistTac) (Abs.TypePart (TAC [] []) (TypeExpression (TAC [] []) (Abs.PrimitiveTypeInt (TAC [] [])))) initTac ,(sel2 exprTac),(sel3 exprTac),dim,addrIdList,[AddrAddress "array"]++(buildInitArray initAddr length))
                                                                                                                                                            Abs.TypeExpressionArray _ _ _ -> (Abs.VariableDeclaration (expression_content (sel1 exprTac)) (sel1 idlistTac) (Abs.TypePart (TAC [] []) (TypeExpression (TAC [] []) (Abs.PrimitiveTypeInt (TAC [] [])))) initTac ,(sel2 exprTac),(sel3 exprTac),dim,addrIdList,[AddrAddress "array"]++(buildInitArray initAddr length))
                                                                                                                                                            _ -> if length>0
                                                                                                                                                                then (Abs.VariableDeclaration (expression_content (sel1 exprTac)) (sel1 idlistTac) (Abs.TypePart (TAC [] []) (TypeExpression (TAC [] []) (Abs.PrimitiveTypeInt (TAC [] [])))) initTac ,(sel2 exprTac),(sel3 exprTac),dim,addrIdList,[AddrAddress "array"]++(buildInitArray initAddr length))
                                                                                                                                                                else (Abs.VariableDeclaration (expression_content (sel1 exprTac)) (sel1 idlistTac) (Abs.TypePart (TAC [] []) (TypeExpression (TAC [] []) (Abs.PrimitiveTypeInt (TAC [] [])))) initTac ,(sel2 exprTac),(sel3 exprTac),-1,addrIdList,[initAddr])
                                                                            Abs.InitializzationPartArray resi array -> let idlistTac = genTacIdentifierList idlist n l k (w,j) in
                                                                                                                            let tacId = identlist_content (sel1 idlistTac) in
                                                                                                                                let addrIdList = sel4 idlistTac in
                                                                                                                                    let dim = generateDimForArray typeExp in
                                                                                                                                        let arrayInitTac = (genTacArrayInit array (sel2 idlistTac) l (sel3 idlistTac) (w,j) dim 0 typeExp) in
...
genTacVarDecId (Abs.VariableDeclarationChecked res@(TResult env ty _) idlist typepart@(Abs.TypePart _ typeExp) initpart) n l k (w,j) = case initpart of
                                                                            Abs.InitializzationPartEmpty resi -> let idlistTac = genTacIdentifierList idlist n l k (w,j) in
                                                                                                                    let tacId = identlist_content (sel1 idlistTac) in
                                                                                                                        let addrIdList = sel4 idlistTac in
                                                                                                                            let initAddr = AddrNULL in
...
genTacArrayInit :: Abs.ARRAYINIT TCheckResult -> Prelude.Integer -> Label -> Prelude.Integer -> (Label,Label) -> Prelude.Integer -> Prelude.Integer -> Abs.TYPEEXPRESSION TCheckResult -> (Abs.ARRAYINIT TAC,Prelude.Integer,Prelude.Integer,[Address],Prelude.Integer)
genTacArrayInit (Abs.ArrayInitElems res listelementarray ) n l k (w,j) dim c typeExp = let listElementTac = genTacListElementOfArray listelementarray n l k (w,j) dim c (getSonTypeExp typeExp) in
                                                                                (Abs.ArrayInitElems (listelementarray_content (sel1 listElementTac)) (sel1 listElementTac),(sel2 listElementTac),(sel3 listElementTac),(sel4 listElementTac),sel5 listElementTac)
genTacArrayInit (Abs.ArrayInitSingle res arrayInit) n l k (w,j) dim c typeExp = let arrayInitTac = genTacArrayInit arrayInit n l k (w,j) dim c (getSonTypeExp typeExp) in
                                                                                    (Abs.ArrayInitSingle (arrayinit_content (sel1 arrayInitTac)) (sel1 arrayInitTac),(sel2 arrayInitTac),(sel3 arrayInitTac),sel4 arrayInitTac,sel5 arrayInitTac)
genTacArrayInit (Abs.ArrayInit res arrayInit1 arrayInit2) n l k (w,j) dim c typeExp = let arrayInitTac1 = genTacArrayInit arrayInit1 n l k (w,j) dim c typeExp in
                                                                            let arrayInitTac2 = genTacArrayInit arrayInit2 (sel2 arrayInitTac1) l (sel3 arrayInitTac1) (w,j) dim (sel5 arrayInitTac1) (getSonTypeExp typeExp) in
                                                                                    (Abs.ArrayInit (mergeTacs [(arrayinit_content (sel1 arrayInitTac1)),
                                                                                                                (arrayinit_content (sel1 arrayInitTac2))]) (sel1 arrayInitTac1) (sel1 arrayInitTac2),(sel2 arrayInitTac2),(sel3 arrayInitTac2),(sel4 arrayInitTac1)++(sel4 arrayInitTac2),sel5 arrayInitTac2)  

genTacListElementOfArray :: Abs.LISTELEMENTARRAY TCheckResult -> Prelude.Integer -> Label -> Prelude.Integer -> (Label,Label) -> Prelude.Integer -> Prelude.Integer -> Abs.TYPEEXPRESSION TCheckResult -> (Abs.LISTELEMENTARRAY TAC,Prelude.Integer,Prelude.Integer,[Address],Prelude.Integer)
genTacListElementOfArray (Abs.ListElementOfArray res exp)  n l k (w,j) dim c typeExp = let expType = getTypeFromExpr exp in
                                                                            if expType==B_type Type_Boolean && isAndOrOp exp  
                                                                                then
                                                                                    let temp = newTemp n in
                                                                                        let label = newLabel "endSet" k in
                                                                                            let expTac = genTacExpression exp (n+1) label (k+1) (w,j) "wor" in
                                                                                                (Abs.ListElementOfArray (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                    (expression_content (sel1 expTac)),
                                                                                                                                    (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                    (TAC [TacLabel label] [])
                                                                                                                                    ]) (sel1 expTac),(sel2 expTac),(sel3 expTac)+1,[temp],c)
                                                                                else
                                                                                    case expType of
                                                                                        Array ta dima -> let expTac = genTacExpression exp n l k (w,j) "" in
                                                                                                            let tacentry = generateTac (sel2 expTac) (sel4 expTac) c dim (genAbsoluteType ta) (getDimFromTypeExp typeExp) in
                                                                                                                (Abs.ListElementOfArray (mergeTacs [(TAC (code (expression_content (sel1 expTac))) []),
                                                                                                                                                    (TAC (sel1 tacentry) [])]) (sel1 expTac),(sel2 tacentry),(sel3 expTac),(sel3 tacentry),sel4 tacentry)
                                                                                        _ -> let expTac = genTacExpression exp n l k (w,j) "" in
                                                                                                (Abs.ListElementOfArray (TAC (code (expression_content (sel1 expTac))) []) (sel1 expTac),(sel2 expTac),(sel3 expTac),[sel4 expTac],c)
genTacListElementOfArray (Abs.ListElementsOfArray res exp listelement)  n l k (w,j) dim c typeExp = let expType = getTypeFromExpr exp in
                                                                                        if expType==B_type Type_Boolean && isAndOrOp exp  
                                                                                            then
                                                                                                let temp = newTemp n in
                                                                                                    let label = newLabel "endSet" k in
                                                                                                        let expTac = genTacExpression exp (n+1) label (k+1) (w,j) "wor" in
                                                                                                            let listelementTac = genTacListElementOfArray listelement (sel2 expTac) l ((sel3 expTac)+1) (w,j) dim c typeExp in
                                                                                                                (Abs.ListElementsOfArray (mergeTacs[ (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                    (expression_content (sel1 expTac)),
                                                                                                                                    (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                    (TAC [TacLabel label] []),
                                                                                                                                    (listelementarray_content (sel1 listelementTac))
                                                                                                                                    ]) (sel1 expTac) (sel1 listelementTac),(sel2 listelementTac),(sel3 listelementTac),[temp]++(sel4 listelementTac),c)
                                                                                            else
                                                                                                case expType of
                                                                                                    Array ta dima ->
                                                                                                        let expTac = genTacExpression exp n l k (w,j) "" in
                                                                                                            let tacentry = generateTac (sel2 expTac) (sel4 expTac) c dim (genAbsoluteType ta) (getDimFromTypeExp typeExp) in
                                                                                                                let listelementTac = genTacListElementOfArray listelement (sel2 tacentry) l (sel3 expTac) (w,j) dim c typeExp in
                                                                                                                    (Abs.ListElementsOfArray (mergeTacs [(TAC (code (expression_content (sel1 expTac))) []),
                                                                                                                                                    (TAC (sel1 tacentry) []),
                                                                                                                                                    (TAC (code (listelementarray_content (sel1 listelementTac))) [])
                                                                                                                                                    ]) (sel1 expTac) (sel1 listelementTac),(sel2 listelementTac),(sel3 listelementTac),(sel3 tacentry)++(sel4 listelementTac),sel5 listelementTac)
                                                                                                    _ ->
                                                                                                        let expTac = genTacExpression exp n l k (w,j) "" in
                                                                                                            let listelementTac = genTacListElementOfArray listelement (sel2 expTac) l (sel3 expTac) (w,j) dim c typeExp in
                                                                                                            (Abs.ListElementsOfArray (TAC ((code (expression_content (sel1 expTac))) ++ (code (listelementarray_content (sel1 listelementTac)))) []) (sel1 expTac) (sel1 listelementTac),(sel2 listelementTac),(sel3 listelementTac),[sel4 expTac]++(sel4 listelementTac),c)
...
genTacExpression (Abs.ExpressionCast res def tipo)                              n l k (w,j) str = let defTac = genTacDefault def n l k (w,j) 0 str in
                                                                                                                ...
                                                                                                                let fromType = getTypeFromDefault def in
                                                                                                                    let toType = TacGen.getTypeFromPrimitive tipo in
                                                                                                                        ((Abs.ExpressionCast (TAC ((code (default_content (sel1 defTac))) ++ -- tac of default (left part)
                                                                                                                                                   [TacCastConversion temp (if (sel4 defTac)==AddrNULL then (newTemp ((sel2 tipoTac)-1)) else sel4 defTac) fromType toType]) -- Tac instruction for conversion (cast)
                                                                                                                                                   []) (sel1 defTac) (sel1 tipoTac)),(sel2 tipoTac)+1,(sel3 tipoTac),temp)
genTacExpression (Abs.ExpressionUnary res@(TResult env ty pos) unary def)       n l k (w,j) str = if isAndOrOp_ def
                                                                                                    then
                                                                                                        case unary of 
                                                                                                            Abs.UnaryOperationPositive _ -> let defTac = genTacDefault def (n+1) l k (w,j) 1 str in 
                                                                                                                                                let defAddr = sel4 defTac in
                                                                                                                                                    let temp = newTemp n in
                                                                                                                                                        (Abs.ExpressionUnary (merge2Tacs (default_content (sel1 defTac)) (TAC [TacAssignUnaryOp temp Pos defAddr ty] [])  ) (Abs.UnaryOperationPositive (TAC [] [])) (sel1 defTac),(sel2 defTac),(sel3 defTac),temp) 
                                                                                                            Abs.UnaryOperationNegative _ ->  let defTac = genTacDefault def (n+1) l k (w,j) 1 str in 
                                                                                                                                                    let temp = newTemp n in
                                                                                                                                                        (Abs.ExpressionUnary (mergeTacs [
                                                                                                                                                                                        (default_content (sel1 defTac)),
                                                                                                                                                                                        (TAC [TacAssignUnaryOp temp Not (sel4 defTac) ty] [])
                                                                                                                                                                                        ]) (Abs.UnaryOperationNegative (TAC [] [])) (sel1 defTac),(sel2 defTac),(sel3 defTac),temp) 
                                                                                                            Abs.UnaryOperationNot      _ -> let defTac = genTacDefault def (n+1) l k (w,j) 1 str in 
                                                                                                                                                let defAddr = sel4 defTac in
                                                                                                                                                    let temp = newTemp n in
                                                                                                                                                        (Abs.ExpressionUnary (merge2Tacs (default_content (sel1 defTac)) (TAC [TacAssignUnaryOp temp Not defAddr ty] [])  ) (Abs.UnaryOperationNot (TAC [] [])) (sel1 defTac),(sel2 defTac),(sel3 defTac),temp) 
                                                                                                            Abs.UnaryOperationPointer  _ -> let defTac = genTacDefault def (n+1) l k (w,j) 1 str in 
                                                                                                                                                let defAddr = sel4 defTac in
                                                                                                                                                    let temp = newTemp n in
                                                                                                                                                        (Abs.ExpressionUnary (merge2Tacs (default_content (sel1 defTac)) (TAC [TacAssignUnaryOp temp Point defAddr ty] [])) (Abs.UnaryOperationPointer (TAC [] [])) (sel1 defTac),(sel2 defTac),(sel3 defTac),temp)
                                                                                                    else
                                                                                                        let defTac = genTacDefault def (n+1) l k (w,j) 1 str in 
                                                                                                            let defAddr = sel4 defTac in
                                                                                                                let temp = newTemp n in
                                                                                                                    case unary of 
...
genTacExpression (Abs.ExpressionBinaryPlus res@(TResult env t pos) expr1 expr2) n l k (w,j) str = if isAndOrOp expr1
                                                                                                    then if isAndOrOp expr2
                                                                                                            then let label = newLabel "endSet" k in
                                                                                                                    let label2 = newLabel "endSet" (k+1) in
                                                                                                                        let expr1Tac = genTacExpression expr1 (n+1) label (k+2) (w,j) "wor" in 
                                                                                                                            let expr2Tac = genTacExpression expr2 ((sel2 expr1Tac)+1) label2 (sel3 expr1Tac) (w,j) "wor" in
                                                                                                                                let tac1Code = code (expression_content (sel1 expr1Tac)) in
                                                                                                                                    let last1El = last tac1Code in
                                                                                                                                        let remain1El = init tac1Code in
                                                                                                                                            let tac2Code = code (expression_content (sel1 expr2Tac)) in
                                                                                                                                                let last2El = last tac2Code in
                                                                                                                                                    let remain2El = init tac2Code in
                                                                                                                                                        let temp = newTemp n in
                                                                                                                                                            let temp2 = newTemp (sel2 expr1Tac) in
                                                                                                                                                                let temp3 = newTemp (sel2 expr2Tac) in
                                                                                                                                                                    case t of
                                                                                                                                                                        B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                                                let remain1El2 = init remain1El in
                                                                                                                                                                                                    let last2El2 = last remain2El in
                                                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                                                            (Abs.ExpressionBinaryPlus (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                                                        TAC [last1El2] [],
                                                                                                                                                                                                                                        TAC [last1El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp2 (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain2El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp2 (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label2] []),
                                                                                                                                                                                                                                        TAC [last2El2] [],
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "plus") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                            ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                                        B_type Type_Integer ->
                                                                                                                                                                                                (Abs.ExpressionBinaryPlus (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El [],
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "plus") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+2,temp3)
                                                                                                            else let label = newLabel "endSet" k in
                                                                                                                    let expr1Tac = genTacExpression expr1 n label (k+1) (w,j) "wor" in 
                                                                                                                        let expr2Tac = genTacExpression expr2 ((sel2 expr1Tac)+1) l (sel3 expr1Tac) (w,j) str in
                                                                                                                            let tac1Code = code (expression_content (sel1 expr1Tac)) in
                                                                                                                                let last1El = last tac1Code in
                                                                                                                                    let remain1El = init tac1Code in
                                                                                                                                        let temp = newTemp (sel2 expr1Tac) in
                                                                                                                                            let temp2 = newTemp (sel2 expr2Tac) in
                                                                                                                                                case t of
                                                                                                                                                B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                        let remain1El2 = init remain1El in
                                                                                                                                                                            (Abs.ExpressionBinaryPlus (mergeTacs [
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                TAC [last1El] [],
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignBinaryOp temp2 (buildOp t "plus") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                        ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                B_type Type_Integer ->
                                                                                                                                                                        (Abs.ExpressionBinaryPlus (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                    ...
                                                                                                    else if isAndOrOp expr2
                                                                                                        then let label = newLabel "endSet" k in
                                                                                                                let expr1Tac = genTacExpression expr1 n l (k+1) (w,j) str in 
                                                                                                                    let expr2Tac = genTacExpression expr2 ((sel2 expr1Tac)+1) label (sel3 expr1Tac) (w,j) "wor" in
                                                                                                                        let tac2Code = code (expression_content (sel1 expr2Tac)) in
                                                                                                                            let last2El = last tac2Code in
                                                                                                                                let remain2El = init tac2Code in
                                                                                                                                    let temp = newTemp (sel2 expr1Tac) in
                                                                                                                                        let temp2 = newTemp (sel2 expr2Tac) in
                                                                                                                                            case t of
                                                                                                                                                B_type Type_Real -> let last2El2 = last remain2El in
                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                            (Abs.ExpressionBinaryPlus (mergeTacs [
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                        ...
                                                                                                        else let expr1Tac = genTacExpression expr1 (n+1) l k (w,j) str in 
...
genTacExpression (Abs.ExpressionBinaryMinus res@(TResult env t pos) expr1 expr2) n l k (w,j) str = if isAndOrOp expr1
                                                                                                    then if isAndOrOp expr2
                                                                                                            then let label = newLabel "endSet" k in
                                                                                                                    let label2 = newLabel "endSet" (k+1) in
                                                                                                                        let expr1Tac = genTacExpression expr1 (n+1) label (k+2) (w,j) "wor" in 
                                                                                                                                                                    ...
                                                                                                                                                                    case t of
                                                                                                                                                                        B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                                                let remain1El2 = init remain1El in
                                                                                                                                                                                                    let last2El2 = last remain2El in
                                                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                                                            (Abs.ExpressionBinaryMinus (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        TAC [last2El2] [],
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "minus") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                            ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                                        B_type Type_Integer ->
                                                                                                                                                                                                (Abs.ExpressionBinaryMinus (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                ...
                                                                                                                                                B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                        let remain1El2 = init remain1El in
                                                                                                                                                                            (Abs.ExpressionBinaryMinus (mergeTacs [
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                TAC [last1El] [],
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignBinaryOp temp2 (buildOp t "minus") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                        ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                B_type Type_Integer ->
                                                                                                                                                                        (Abs.ExpressionBinaryMinus (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
...
genTacExpression (Abs.ExpressionBinaryProduct res@(TResult env t pos) expr1 expr2) n l k (w,j) str = if isAndOrOp expr1
                                                                                                    then if isAndOrOp expr2
                                                                                                            then let label = newLabel "endSet" k in
                                                                                                                    let label2 = newLabel "endSet" (k+1) in
                                                                                                                        let expr1Tac = genTacExpression expr1 (n+1) label (k+2) (w,j) "wor" in 
                                                                                                                                                                    ...
                                                                                                                                                                    case t of
                                                                                                                                                                        B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                                                let remain1El2 = init remain1El in
                                                                                                                                                                                                    let last2El2 = last remain2El in
                                                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                                                            (Abs.ExpressionBinaryProduct (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        TAC [last2El2] [],
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "product") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                            ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                                        B_type Type_Integer ->
                                                                                                                                                                                                (Abs.ExpressionBinaryProduct (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                ...
                                                                                                                                                B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                        let remain1El2 = init remain1El in
                                                                                                                                                                            (Abs.ExpressionBinaryProduct (mergeTacs [
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                TAC [last1El] [],
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignBinaryOp temp2 (buildOp t "product") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                        ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                B_type Type_Integer ->
                                                                                                                                                                        (Abs.ExpressionBinaryProduct (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
...
genTacExpression (Abs.ExpressionBinaryDivision res@(TResult env t pos) expr1 expr2) n l k (w,j) str = if isAndOrOp expr1
                                                                                                    then if isAndOrOp expr2
                                                                                                            then let label = newLabel "endSet" k in
                                                                                                                    let label2 = newLabel "endSet" (k+1) in
                                                                                                                        let expr1Tac = genTacExpression expr1 (n+1) label (k+2) (w,j) "wor" in 
                                                                                                                                                                    ...
                                                                                                                                                                    case t of
                                                                                                                                                                        B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                                                let remain1El2 = init remain1El in
                                                                                                                                                                                                    let last2El2 = last remain2El in
                                                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                                                            (Abs.ExpressionBinaryDivision (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        TAC [last2El2] [],
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "division") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                            ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                                        B_type Type_Integer ->
                                                                                                                                                                                                (Abs.ExpressionBinaryDivision (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                ...
                                                                                                                                                B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                        let remain1El2 = init remain1El in
                                                                                                                                                                            (Abs.ExpressionBinaryDivision (mergeTacs [
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                TAC [last1El] [],
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignBinaryOp temp2 (buildOp t "division") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                        ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                B_type Type_Integer ->
                                                                                                                                                                        (Abs.ExpressionBinaryDivision (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
...
genTacExpression (Abs.ExpressionBinaryModule res@(TResult env t pos) expr1 expr2) n l k (w,j) str = if isAndOrOp expr1
                                                                                                    then if isAndOrOp expr2
                                                                                                            then let label = newLabel "endSet" k in
                                                                                                                    let label2 = newLabel "endSet" (k+1) in
                                                                                                                        let expr1Tac = genTacExpression expr1 (n+1) label (k+2) (w,j) "wor" in 
                                                                                                                                                                    ...
                                                                                                                                                                    case t of
                                                                                                                                                                        B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                                                let remain1El2 = init remain1El in
                                                                                                                                                                                                    let last2El2 = last remain2El in
                                                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                                                            (Abs.ExpressionBinaryModule (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        TAC [last2El2] [],
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "module") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                            ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                                        B_type Type_Integer ->
                                                                                                                                                                                                (Abs.ExpressionBinaryModule (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                ...
                                                                                                                                                B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                        let remain1El2 = init remain1El in
                                                                                                                                                                            (Abs.ExpressionBinaryModule (mergeTacs [
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                TAC [last1El] [],
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignBinaryOp temp2 (buildOp t "module") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                        ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                B_type Type_Integer ->
                                                                                                                                                                        (Abs.ExpressionBinaryModule (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
...
genTacExpression (Abs.ExpressionBinaryPower res@(TResult env t pos) expr1 expr2) n l k (w,j) str = if isAndOrOp expr1
                                                                                                    then if isAndOrOp expr2
                                                                                                            then let label = newLabel "endSet" k in
                                                                                                                    let label2 = newLabel "endSet" (k+1) in
                                                                                                                        let expr1Tac = genTacExpression expr1 (n+1) label (k+2) (w,j) "wor" in 
                                                                                                                                                                    ...
                                                                                                                                                                    case t of
                                                                                                                                                                        B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                                                let remain1El2 = init remain1El in
                                                                                                                                                                                                    let last2El2 = last remain2El in
                                                                                                                                                                                                        let remain2El2 = init remain2El in
                                                                                                                                                                                                            (Abs.ExpressionBinaryPower (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        TAC remain1El2 [],
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool False) (B_type Type_Boolean)] []),
                                                                                                                                                                                                                                        (TAC [TacLabel label] []),
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        TAC [last2El2] [],
                                                                                                                                                                                                                                        TAC [last2El] [],
                                                                                                                                                                                                                                        (TAC [TacAssignBinaryOp temp3 (buildOp t "power") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                                                            ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                                        B_type Type_Integer ->
                                                                                                                                                                                                (Abs.ExpressionBinaryPower (mergeTacs [
                                                                                                                                                                                                                                        (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
                                                                                                                                                ...
                                                                                                                                                B_type Type_Real -> let last1El2 = last remain1El in
                                                                                                                                                                        let remain1El2 = init remain1El in
                                                                                                                                                                            (Abs.ExpressionBinaryPower (mergeTacs [
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                TAC [last1El] [],
                                                                                                                                                                                                                (expression_content (sel1 expr1Tac)),
                                                                                                                                                                                                                (TAC [TacAssignBinaryOp temp2 (buildOp t "power") (sel4 expr1Tac) (sel4 expr2Tac) t] [])
                                                                                                                                                                        ]) (sel1 expr1Tac) (sel1 expr2Tac),(sel2 expr2Tac)+1,(sel3 expr2Tac)+1,temp2)
                                                                                                                                                B_type Type_Integer ->
                                                                                                                                                                        (Abs.ExpressionBinaryPower (mergeTacs [
                                                                                                                                                                                                                (TAC [TacAssignNullOp False temp (AddrBool True) (B_type Type_Boolean)] []),
...
genTacExpression (Abs.ExpressionIdent res ident@(Abs.Ident id resi@(TResult env t pos)) index) n l k (w,j) str = case Data.Map.lookup id env of
                                                                                                                  Just (entry:entries) -> case findEntryOfType (entry:entries) "var" of
                                                                                                                                            (x@(Variable _ posv _ _ _ _):xs) -> case index of
                                                                                                                                                                            (Abs.ArrayIndexElementEmpty _) -> ((Abs.ExpressionIdent (TAC [] []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElementEmpty (TAC [] []))),n,k,buildIDAddr posv id env)
                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                        let leftAddrArray = buildArrayId leftAddr posArr 0 in
                                                                                                                                                                                                                                                                                            let comment = buildArrayComments (sel4 typeIndexTac) s dim t in
                                                                                                                                                                                                                                                                                                let temp = newTemp (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                                    ((Abs.ExpressionIdent (TAC ([TacAssignNullOp False temp leftAddrArray t]++comment) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElement (TAC [] []) (sel1 typeIndexTac))),(sel2 typeIndexTac)+1,(sel3 typeIndexTac),temp)
                                                                                                                                                                                                                                                                                else
                                                                                                                                                                                                                                                                                    let typeIndexTac = genTacTypeIndex typeindex n l k (w,j) dim s (-1) in
                                                                                                                                                                                                                                                                                        let posArr = buildPosArrayNoInteger (sel4 typeIndexTac) s dim (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                            let leftAddrArray = buildArrayIdNoInteger leftAddr (newTemp ((sel2 typeIndexTac)-1)) in
                                                                                                                                                                                                                                                                                                let temp = newTemp (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                                    ((Abs.ExpressionIdent (TAC ((code (typeindex_content (sel1 typeIndexTac)))++[TacAssignNullOp False temp leftAddrArray t]) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElement (TAC [] []) (sel1 typeIndexTac))),(sel2 typeIndexTac)+1,(sel3 typeIndexTac),temp)                                                                         
                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                        let posArr = buildPosArray ((sel4 arrayIndexTac)++(sel4 typeIndexTac)) s dim in
                                                                                                                                                                                                                                                                                                            let leftAddrArray = buildArrayId leftAddr posArr 0 in
                                                                                                                                                                                                                                                                                                                let comment = buildArrayComments ((sel4 arrayIndexTac)++ (sel4 typeIndexTac)) s dim realT in
                                                                                                                                                                                                                                                                                                                    let temp = newTemp (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                        ((Abs.ExpressionIdent (TAC ([TacAssignNullOp False temp leftAddrArray realT]++comment) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElements (TAC [] [])  (sel1 arrayIndexTac) (sel1 typeIndexTac))),(sel2 arrayIndexTac)+1,(sel3 arrayIndexTac),temp)
                                                                                                                                                                                                                                                                                                    else
                                                                                                                                                                                                                                                                                                        let arrayIndexTac = genTacArrayIndexElements (Abs.ArrayIndexElementsMultiple res arrayIndex typeindex) n l k (w,j) dim s (-1) in
                                                                                                                                                                                                                                                                                                            let posArr = buildPosArrayNoInteger (sel4 arrayIndexTac) s dim (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                let leftAddrArray = buildArrayIdNoInteger leftAddr (newTemp ((sel2 arrayIndexTac)-1)) in
                                                                                                                                                                                                                                                                                                                        let temp = newTemp (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                            ((Abs.ExpressionIdent (TAC ((code (arrayindexelements_content (sel1 arrayIndexTac)))++[TacAssignNullOp False temp leftAddrArray realT]) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElements (TAC [] [])  (sel1 arrayIndexTac) (sel1 typeIndexTac))),(sel2 arrayIndexTac)+1,(sel3 arrayIndexTac),temp)
...
genTacExpression (Abs.ExpressionCall res@(TResult env _ _) ident@(Abs.Ident id resi) exps) n l k (w,j) str = case Data.Map.lookup id env of 
                                                                                                                Just (entry:entries) -> case findEntryOfType (entry:entries) "func" of
                                                                                                                                          ((Function ty pos params@(p:ps) _):xs) -> let funcReturn = newTemp n in
                                                                                                                                                                                      let funcAddr = buildIDAddr pos id env in
                                                                                                                                                                                      case exps of
                                                                                                                                                                                          (Abs.Expressions rese expr exprs) -> let expression = genTacExpression expr (n+1) l k (w,j) str in
                                                                                                                                                                                                                                let expressions = genTacExpressions exprs (tail params) (sel2 expression) l (sel3 expression) (w,j) in
                                                                                                                                                                                                                                    case p of
                                                                                                                                                                                                                                      (TypeChecker.Parameter _ _ "val" _) -> ((Abs.ExpressionCall (TAC ((code (expression_content (sel1 expression))) ++         -- evaluation of the first param value
                                                                                                                                                                                                                                                                                                        [TacParam (sel4 expression) (getTypeFromExpr expr) False] ++  -- first param tac
                                                                                                                                                                                                                                                                                                        (code (expressions_content (sel1 expressions))) ++       -- tac of others param
                                                                                                                                                                                                                                                                                                        [TacAssignNullOp True funcReturn funcAddr ty])            -- call tac
                                                                                                                                                                                                                                                                                                        []) (Abs.Ident id (TAC [][])) (Abs.Expressions (TAC [][]) (sel1 expression) (sel1 expressions))),(sel2 expressions),(sel3 expressions),funcReturn) 
                                                                                                                                                                                                                                      (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp (sel2 expressions) in ((Abs.ExpressionCall (TAC ((code (expression_content (sel1 expression))) ++      -- evaluation of the first param value
                                                                                                                                                                                                                                                                                                                                                    [TacPointRef temp (sel4 expression),
                                                                                                                                                                                                                                                                                                                                                    TacComment "valres-mode",
                                                                                                                                                                                                                                                                                                                                                    (TacParam  temp (getTypeFromExpr expr) True)] ++     -- first param tac
                                                                                                                                                                                                                                                                                                                                                    (code (expressions_content (sel1 expressions))) ++   -- tac of others param
                                                                                                                                                                                                                                                                                                                                                    [TacAssignNullOp True funcReturn funcAddr ty])       -- call tac
                                                                                                                                                                                                                                                                                                                                                    []) (Abs.Ident id (TAC [][])) (Abs.Expressions (TAC [][]) (sel1 expression) (sel1 expressions))),(sel2 expressions)+1,(sel3 expressions),funcReturn) 
                                                                                                                                                                                          (Abs.Expression rese expr)        -> let expression = genTacExpression expr (n+1) l k (w,j) str in
                                                                                                                                                                                                                                  case p of 
                                                                                                                                                                                                                                      (TypeChecker.Parameter _ _ "val" _) -> ((Abs.ExpressionCall (TAC ((code (expression_content (sel1 expression))) ++        -- evaluation of the param value
                                                                                                                                                                                                                                                                                                  [(TacParam (sel4 expression) (getTypeFromExpr expr) False),    -- param tac
                                                                                                                                                                                                                                                                                                   (TacAssignNullOp True funcReturn funcAddr ty)])              -- call tac
                                                                                                                                                                                                                                                                                                   []) (Abs.Ident id (TAC [][])) (Abs.Expression (TAC [][]) (sel1 expression))),(sel2 expression),(sel3 expression),funcReturn) 
                                                                                                                                                                                                                                      (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp (sel2 expression) in ((Abs.ExpressionCall (TAC ((code (expression_content (sel1 expression))) ++     -- evaluation of the param value
                                                                                                                                                                                                                                                                                                                                                  [TacPointRef temp (sel4 expression),
                                                                                                                                                                                                                                                                                                                                                   TacComment "valres-mode",
                                                                                                                                                                                                                                                                                                                                                   (TacParam  temp (getTypeFromExpr expr) True),    -- param tac
                                                                                                                                                                                                                                                                                                                                                   (TacAssignNullOp True funcReturn funcAddr ty)])   -- call tac
                                                                                                                                                                                                                                                                                                                                                   []) (Abs.Ident id (TAC [][])) (Abs.Expression (TAC [][]) (sel1 expression))),(sel2 expression)+1,(sel3 expression),funcReturn) 
                                                                                                                                                                                          (Abs.ExpressionEmpty rese)        -> ((Abs.ExpressionCall (TAC [TacAssignNullOp True funcReturn funcAddr ty]   -- call tac
                                                                                                                                                                                                                                                           []) (Abs.Ident id (TAC [][])) (Abs.ExpressionEmpty (TAC [][]))),n+1,k,funcReturn) 
                                                                                                                                          ((Function ty pos [] _):xs) ->  let funcReturn = newTemp n in
                                                                                                                                                                            let funcAddr = buildIDAddr pos id env in   
                                                                                                                                                                                ((Abs.ExpressionCall (TAC [TacAssignNullOp True funcReturn funcAddr ty]   -- call tac
                                                                                                                                                                                                                                                           []) (Abs.Ident id (TAC [][])) (Abs.ExpressionEmpty (TAC [][]))),n+1,k,funcReturn)                                                                                                       

-- for functions R calls (expressions param lists)
genTacExpressions :: Abs.EXPRESSIONS TCheckResult -> [TypeChecker.Parameter] -> Prelude.Integer -> Label -> Prelude.Integer -> (Label,Label) -> (Abs.EXPRESSIONS TAC,Prelude.Integer,Prelude.Integer,Address)
genTacExpressions (Abs.Expressions res expr exprs) (p:ps) n l k (w,j) = let expression = genTacExpression expr n l k (w,j) "" in 
                                                                            let expressions = genTacExpressions exprs ps (sel2 expression) l (sel3 expression) (w,j) in
                                                                                case p of 
                                                                                    (TypeChecker.Parameter _ _ "val" _) -> ((Abs.Expressions (TAC ((code (expression_content (sel1 expression))) ++     
                                                                                                                                                    [TacParam (sel4 expression) (getTypeFromExpr expr) False] ++
                                                                                    ...
                                                                                    (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp (sel2 expressions) in 
                                                                                                                                            ((Abs.Expressions (TAC ((code (expression_content (sel1 expression))) ++     
                                                                                                                                                                   [TacPointRef temp (sel4 expression),
                                                                                                                                                                   TacComment "valres-mode",
                                                                                                                                                                   (TacParam temp (getTypeFromExpr expr) True)] ++
                                                                                                                                                                   (code (expressions_content (sel1 expressions))))
                                                                                                                                                                   []) (sel1 expression) (sel1 expressions)),(sel2 expressions),(sel3 expressions)+1,(sel4 expression))  
                                                                                
genTacExpressions (Abs.Expression res expr) (p:ps) n l k (w,j) = let expression = genTacExpression expr n l k (w,j) "" in 
                                                                    case p of 
                                                                        (TypeChecker.Parameter _ _ "val" _) -> ((Abs.Expression (TAC ((code (expression_content (sel1 expression))) ++     
                                                                                                                                        [TacParam (sel4 expression) (getTypeFromExpr expr) False]) 
                                                                                                                                        []) (sel1 expression)),(sel2 expression),(sel3 expression),(sel4 expression))
                                                                        (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp (sel2 expression) in 
                                                                                                                        ((Abs.Expression (TAC ((code (expression_content (sel1 expression))) ++     
                                                                                                                                                [TacPointRef temp (sel4 expression),
                                                                                                                                                 TacComment "valres-mode",
                                                                                                                                                 (TacParam temp (getTypeFromExpr expr) True)]) 
                                                                                                                                                 []) (sel1 expression)),(sel2 expression),(sel3 expression)+1,(sel4 expression))
genTacExpressions (Abs.ExpressionEmpty res) (p:ps) n l k (w,j)  = ((Abs.ExpressionEmpty (TAC [][])),n,k,AddrNULL)
...
genTacDefault :: Abs.DEFAULT TCheckResult -> Prelude.Integer -> Label -> Prelude.Integer -> (Label,Label) -> Prelude.Integer -> Prelude.String -> (Abs.DEFAULT TAC,Prelude.Integer,Prelude.Integer,Address)
genTacDefault (Abs.ExpressionIntegerD res value@(Abs.Integer val resi))       n l k (w,j) d str = (Abs.ExpressionIntegerD (TAC [] []) (Abs.Integer val   (TAC [] [])),n,k, AddrInt val)
genTacDefault (Abs.ExpressionBooleanD res value@(Abs.Boolean_true resi))      n l k (w,j) d str = (Abs.ExpressionBooleanD (TAC [] []) (Abs.Boolean_true  (TAC [] [])),n,k, AddrBool True)
genTacDefault (Abs.ExpressionBooleanD res value@(Abs.Boolean_false resi))     n l k (w,j) d str = (Abs.ExpressionBooleanD (TAC [] []) (Abs.Boolean_false (TAC [] [])),n,k, AddrBool False)
genTacDefault (Abs.ExpressionBooleanD res value@(Abs.Boolean_True resi))      n l k (w,j) d str = (Abs.ExpressionBooleanD (TAC [] []) (Abs.Boolean_True  (TAC [] [])),n,k, AddrBool True)
genTacDefault (Abs.ExpressionBooleanD res value@(Abs.Boolean_False resi))     n l k (w,j) d str = (Abs.ExpressionBooleanD (TAC [] []) (Abs.Boolean_False (TAC [] [])),n,k, AddrBool False)
genTacDefault (Abs.ExpressionCharD res value@(Abs.Char val resi))             n l k (w,j) d str = (Abs.ExpressionCharD    (TAC [] []) (Abs.Char val      (TAC [] [])),n,k, AddrChar val)
genTacDefault (Abs.ExpressionStringD res value@(Abs.String val resi))         n l k (w,j) d str = (Abs.ExpressionStringD  (TAC [] []) (Abs.String val    (TAC [] [])),n,k, AddrString val)
genTacDefault (Abs.ExpressionRealD res value@(Abs.Real val resi))             n l k (w,j) d str = (Abs.ExpressionRealD    (TAC [] []) (Abs.Real val      (TAC [] [])),n,k, AddrReal val)
genTacDefault (Abs.ExpressionBracketD res exp)                                n l k (w,j) d str = let exprTac = genTacExpression exp n l k (w,j) str in (Abs.ExpressionBracketD (expression_content (sel1 exprTac)) (sel1 exprTac),(sel2 exprTac),(sel3 exprTac), (sel4 exprTac))
genTacDefault (Abs.ExpressionCastD res def tipo)                              n l k (w,j) d str = let defTac = genTacDefault def n l k (w,j) d str in
                                                                                                    let tipoTac = genTacTipo tipo (sel2 defTac) l (sel3 defTac) (w,j) in
                                                                                                        let temp = newTemp (sel2 tipoTac) in
                                                                                                            let fromType = getTypeFromDefault def in
                                                                                                                let toType = TacGen.getTypeFromPrimitive tipo in
                                                                                                                    ((Abs.ExpressionCastD (TAC ((code (default_content (sel1 defTac))) ++ -- tac of default (left part)
                                                                                                                                                ...
                                                                                                                                                []) (sel1 defTac) (sel1 tipoTac)),(sel2 tipoTac)+1,(sel3 tipoTac),temp) 
genTacDefault (Abs.ExpressionUnaryD res@(TResult env ty pos) unary def)       n l k (w,j) d str = let defTac = genTacDefault def (n+1) l k (w,j) (d+1) str in 
                                                                                                let temp = newTemp n in
                                                                                                    case unary of 
...
genTacDefault (Abs.ExpressionIdentD res@(TResult env _ _) ident@(Abs.Ident id resI) index)  n l k (w,j) d str = case Data.Map.lookup id env of
                                                                                                                    Just (entry:entries) -> case findEntryOfType (entry:entries) "var" of
                                                                                                                                              (x@(Variable _ posv _ _ _ _):xs) -> case index of
                                                                                                                                                                                    (Abs.ArrayIndexElementEmpty _) -> ((Abs.ExpressionIdentD (TAC [] []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElementEmpty (TAC [] []))),n,k,buildIDAddr posv id env)
                                                                                                                                                                                    (Abs.ArrayIndexElement _ typeindex) -> case findArray (x:xs) of
                                                                                                                                                                                                                        [Variable ty@(Array t _) posv _ _ s check] -> let typeIndexTac = genTacTypeIndex typeindex n l k (w,j) 0 [] (-1) in
                                                                                                                                                                                                                                                                    let dim = generateDimForArrayFromType ty in
                                                                                                                                                                                                                                                                        let leftAddr = buildIDAddr posv id env in
                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                then
                                                                                                                                                                                                                                                                                    let posArr = buildPosArray (sel4 typeIndexTac) s dim in
                                                                                                                                                                                                                                                                                        let leftAddrArray = buildArrayId leftAddr posArr 0 in
                                                                                                                                                                                                                                                                                            let comment = buildArrayComments (sel4 typeIndexTac) s dim t in
                                                                                                                                                                                                                                                                                                let temp = newTemp (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                                    ((Abs.ExpressionIdentD (TAC ([TacAssignNullOp False temp leftAddrArray t]++comment) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElement (TAC [] []) (sel1 typeIndexTac))),(sel2 typeIndexTac)+1,(sel3 typeIndexTac),temp)
                                                                                                                                                                                                                                                                                else
                                                                                                                                                                                                                                                                                    let typeIndexTac = genTacTypeIndex typeindex n l k (w,j) dim s (-1) in
                                                                                                                                                                                                                                                                                        let posArr = buildPosArrayNoInteger (sel4 typeIndexTac) s dim (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                            let leftAddrArray = buildArrayIdNoInteger leftAddr (newTemp ((sel2 typeIndexTac)-1)) in
                                                                                                                                                                                                                                                                                                let temp = newTemp (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                                    ((Abs.ExpressionIdentD (TAC ((code (typeindex_content (sel1 typeIndexTac)))++[TacAssignNullOp False temp leftAddrArray t]) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElement (TAC [] []) (sel1 typeIndexTac))),(sel2 typeIndexTac)+1,(sel3 typeIndexTac),temp)                                                                         
                                                                                                                                                                                                                        [Variable ty@(Pointer t _) posv _ _ s check] -> let realT = getRealTacType t in
                                                                                                                                                                                                                                                                    let typeIndexTac = genTacTypeIndex typeindex n l k (w,j) 0 [] (-1) in
                                                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                                                    then
                                                                                                                                                                                                                                                                                                        let posArr = buildPosArray ((sel4 arrayIndexTac)++(sel4 typeIndexTac)) s dim in
                                                                                                                                                                                                                                                                                                            let leftAddrArray = buildArrayId leftAddr posArr 0 in
                                                                                                                                                                                                                                                                                                                let comment = buildArrayComments ((sel4 arrayIndexTac)++(sel4 typeIndexTac)) s dim realT in
                                                                                                                                                                                                                                                                                                                    let temp = newTemp (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                        ((Abs.ExpressionIdentD (TAC ([TacAssignNullOp False temp leftAddrArray realT]++comment) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElements (TAC [] [])  (sel1 arrayIndexTac) (sel1 typeIndexTac))),(sel2 arrayIndexTac)+1,(sel3 arrayIndexTac),temp)
                                                                                                                                                                                                                                                                                                    else
                                                                                                                                                                                                                                                                                                        let arrayIndexTac = genTacArrayIndexElements (Abs.ArrayIndexElementsMultiple res arrayIndex typeindex) n l k (w,j) dim s (-1) in
                                                                                                                                                                                                                                                                                                            let posArr = buildPosArrayNoInteger (sel4 arrayIndexTac) s dim (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                let leftAddrArray = buildArrayIdNoInteger leftAddr (newTemp ((sel2 arrayIndexTac)-1)) in
                                                                                                                                                                                                                                                                                                                        let temp = newTemp (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                            ((Abs.ExpressionIdentD (TAC ((code (arrayindexelements_content (sel1 arrayIndexTac)))++[TacAssignNullOp False temp leftAddrArray realT]) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElements (TAC [] [])  (sel1 arrayIndexTac) (sel1 typeIndexTac))),(sel2 arrayIndexTac)+1,(sel3 arrayIndexTac),temp)
                                                                                                                                                                                                                                    [Variable ty@(Pointer t _) posv _ _ s check] -> let realT = getRealTacType t in
                                                                                                                                                                                                                                                                                let typeIndexTac = genTacTypeIndex typeindex n l k (w,j) 0 [] (-1) in
                                                                                                                                                                                                                                                                                    let arrayIndexTac = genTacArrayIndexElements arrayIndex (sel2 typeIndexTac) l (sel3 typeIndexTac) (w,j) 0 [] (-1) in
                                                                                                                                                                                                                                                                                        let dim = generateDimForArrayFromType t in
                                                                                                                                                                                                                                                                                            let leftAddr = buildIDAddr posv id env in
                                                                                                                                                                                                                                                                                                if (isIntegerAddress (sel4 typeIndexTac)) 
                                                                                                                                                                                                                                                                                                    then
                                                                                                                                                                                                                                                                                                        let posArr = buildPosArray ((sel4 arrayIndexTac)++(sel4 typeIndexTac)) s dim in
                                                                                                                                                                                                                                                                                                            let leftAddrArray = buildArrayId leftAddr posArr 0 in
                                                                                                                                                                                                                                                                                                                let comment = buildArrayComments ((sel4 arrayIndexTac)++(sel4 typeIndexTac)) s dim t in
                                                                                                                                                                                                                                                                                                                    let temp = newTemp (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                                                        ((Abs.ExpressionIdentD (TAC ([TacAssignBinaryOp temp AddrIntAdd leftAddr (AddrInt (buildPosArray ((sel4 arrayIndexTac)++(sel4 typeIndexTac)) s dim)) (Pointer t 0)]++comment) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElements (TAC [] []) (sel1 arrayIndexTac) (sel1 typeIndexTac))),(sel2 arrayIndexTac)+1,(sel3 arrayIndexTac),temp)
                                                                                                                                                                                                                                                                                                    else
                                                                                                                                                                                                                                                                                                        let arrayIndexTac = genTacArrayIndexElements (Abs.ArrayIndexElementsMultiple res arrayIndex typeindex) n l k (w,j) dim s (-1) in
                                                                                                                                                                                                                                                                                                            let posArr = buildPosArrayNoInteger (sel4 arrayIndexTac) s dim (sel2 arrayIndexTac) in
                                                                                                                                                                                                                                                                                                                let leftAddrArray = buildArrayIdNoInteger leftAddr (newTemp ((sel2 arrayIndexTac)-1)) in
                                                                                                                                                                                                                                                                                                                    let temp = newTemp (sel2 typeIndexTac) in
                                                                                                                                                                                                                                                                                                                        ((Abs.ExpressionIdentD (TAC ((code (arrayindexelements_content (sel1 arrayIndexTac)))++[TacAssignBinaryOp temp AddrIntAdd leftAddr (head (sel4 typeIndexTac)) (Pointer t 0)]) []) (Abs.Ident id (TAC [] [])) (Abs.ArrayIndexElements (TAC [] []) (sel1 arrayIndexTac) (sel1 typeIndexTac))),(sel2 arrayIndexTac)+1,(sel3 arrayIndexTac),temp)
genTacDefault (Abs.ExpressionCallD res@(TResult env _ _) ident@(Abs.Ident id resI) exps)    n l k (w,j) d str = case Data.Map.lookup id env of
                                                                                                         Just (entry:entries) -> case findEntryOfType (entry:entries) "func" of
                                                                                                            ((Function ty pos params@(p:ps) _):xs) -> let funcReturn = newTemp n in
                                                                                                                                                        let funcAddr = buildIDAddr pos id env in
                                                                                                                                                        case exps of
                                                                                                                                                            (Abs.Expressions rese expr exprs) -> let expression = genTacExpression expr (n+1) l k (w,j) "" in
                                                                                                                                                                                                  let expressions = genTacExpressions exprs (tail params) (sel2 expression) l (sel3 expression) (w,j) in
                                                                                                                                                                                                        case p of
                                                                                                                                                                                                            (TypeChecker.Parameter _ _ "val" _) -> ((Abs.ExpressionCallD (TAC ((code (expression_content (sel1 expression))) ++          -- evaluation of the first param value
                                                                                                                                                                                                                                                                                [TacParam (sel4 expression) (getTypeFromExpr expr) False] ++  -- first param tac
                                                                                                                                                                                                                                                                                (code (expressions_content (sel1 expressions))) ++      -- tac of others param
                                                                                                                                                                                                                                                                                [TacAssignNullOp True funcReturn funcAddr ty])          -- call tac
                                                                                                                                                                                                                                                                                []) (Abs.Ident id (TAC [][])) (Abs.Expressions (TAC [][]) (sel1 expression) (sel1 expressions))),(sel2 expressions),(sel3 expressions),funcReturn) 
                                                                                                                                                                                                            (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp n in ((Abs.ExpressionCallD (TAC ((code (expression_content (sel1 expression))) ++       -- evaluation of the first param value
                                                                                                                                                                                                                                                                                                          [TacPointRef temp (sel4 expression),
                                                                                                                                                                                                                                                                                                          TacComment "valres-mode",
                                                                                                                                                                                                                                                                                                          (TacParam  temp (getTypeFromExpr expr) True)] ++  -- first param tac
                                                                                                                                                                                                                                                                                                          (code (expressions_content (sel1 expressions))) ++     -- tac of others param
                                                                                                                                                            ...
                                                                                                                                                            (Abs.Expression rese expr)        -> let expression = genTacExpression expr (n+1) l k (w,j) "" in
                                                                                                                                                                                                    case p of 
                                                                                                                                                                                                       (TypeChecker.Parameter _ _ "val" _) -> ((Abs.ExpressionCallD (TAC ((code (expression_content (sel1 expression))) ++        -- evaluation of the param value
                                                                                                                                                                                                                                                                            [(TacParam (sel4 expression) (getTypeFromExpr expr) False),   -- param tac
                                                                                                                                                                                                                                                                             (TacAssignNullOp True funcReturn funcAddr ty)])              -- call tac
                                                                                                                                                                                                                                                                             []) (Abs.Ident id (TAC [][])) (Abs.Expression (TAC [][]) (sel1 expression))),(sel2 expression),(sel3 expression),funcReturn) 
                                                                                                                                                                                                       (TypeChecker.Parameter _ _ "valres" _) -> let temp = newTemp n in ((Abs.ExpressionCallD (TAC ((code (expression_content (sel1 expression))) ++        -- evaluation of the param value
                                                                                                                                                                                                                                                                                                    [TacPointRef temp (sel4 expression),
                                                                                                                                                                                                                                                                                                    TacComment "valres-mode",
                                                                                                                                                                                                                                                                                                    (TacParam  temp (getTypeFromExpr expr) True),    -- param tac
                                                                                                                                                                                                                                                                                                    (TacAssignNullOp True funcReturn funcAddr ty)])  -- call tac
                                                                                                                                                                                                                                                                                                    []) (Abs.Ident id (TAC [][])) (Abs.Expression (TAC [][]) (sel1 expression))),(sel2 expression)+1,(sel3 expression),funcReturn)                           
                                                                                                                                                            (Abs.ExpressionEmpty rese)        -> ((Abs.ExpressionCallD (TAC [TacAssignNullOp True funcReturn funcAddr ty]   -- call tac
                                                                                                                                                                                                                             []) (Abs.Ident id (TAC [][])) (Abs.ExpressionEmpty (TAC [][]))),n+1,k,funcReturn)
                                                                                                            ((Function ty pos [] _):xs) ->  let funcReturn = newTemp n in
                                                                                                                                                let funcAddr = buildIDAddr pos id env in    
                                                                                                                                                    ((Abs.ExpressionCallD (TAC [TacAssignNullOp True funcReturn funcAddr ty]   -- call tac
                                                                                                                                                                                                                             []) (Abs.Ident id (TAC [][])) (Abs.ExpressionEmpty (TAC [][]))),n+1,k,funcReturn)    
...
genTacIdentifierList (Abs.IdentifierSingle res@(TResult env _ _) ident@(Abs.Ident id resi@(TResult _ _ pos))) n l k (w,j) = (Abs.IdentifierSingle (TAC [] []) (Abs.Ident id (TAC [] [])),n,k,[buildIDAddr pos id env])
genTacIdentifierList (Abs.IdentifierList res@(TResult env _ _) ident@(Abs.Ident id resi@(TResult _ _ pos)) idlist) n l k (w,j) = let idlistTac = (genTacIdentifierList idlist n l k (w,j)) in
                                                                                                                        let idlistAddr = sel4 idlistTac in
                                                                                                                            (Abs.IdentifierList (TAC [] []) (Abs.Ident id (TAC [] [])) (sel1 idlistTac),(sel2 idlistTac),(sel3 idlistTac),[buildIDAddr pos id env] ++ idlistAddr)
